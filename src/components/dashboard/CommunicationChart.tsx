  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      ...
      try {
        ...
        const res = await http.get<PlatformRow[]>("/interactions/counts/", {
          params: {
            clinic_id: clinicId,
        ...
        const json: PlatformRow[] = Array.isArray(payload)
          ? payload
          : payload &&
              typeof payload === "object" &&
              Array.isArray((payload as { results?: unknown[] }).results)
            ? ((payload as { results: PlatformRow[] }).results ?? [])
            : [];
