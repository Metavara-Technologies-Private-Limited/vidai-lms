...
import type { EmailTemplate, EmailTemplatePayload, LeadMailListItem } from "../../services/leads.api";
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: "16px" } }}>
      ...
      <DialogContent>
        <Stack spacing={2} pt={0.5}>
          ...
          <TextField label="Body" value={body} onChange={(e) => setBody(e.target.value)} size="small" fullWidth required multiline minRows={5} placeholder="Use {{lead_first_name}}, {{appointment_date}}, etc." />
        </Stack>
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 3, pt: 1, gap: 1 }}>
        <Button onClick={onClose} sx={{ height: 38, color: "#374151", fontWeight: 500, textTransform: "none", borderRadius: "8px", border: "1px solid #E5E7EB", px: 3, "&:hover": { bgcolor: "#F3F4F6" } }}>Cancel</Button>
        <Button onClick={handleSave} disabled={saving} variant="contained" sx={{ height: 38, backgroundColor: "#1F2937", color: "white", fontWeight: 500, textTransform: "none", borderRadius: "8px", px: 3, "&:hover": { backgroundColor: "#111827" } }}>
...
// ─────────────────────────────────────────────────────────────────────────────
// EmailDialog  — onSent fires ONLY on successful send
// ─────────────────────────────────────────────────────────────────────────────
interface EmailDialogProps {
  open: boolean;
  lead: LeadRecord | null;
  onClose: () => void;
  onSent?: () => void; // ← NEW: called after successful send
}

const EmailDialog: React.FC<EmailDialogProps> = ({ open, lead, onClose, onSent }) => {
  ...
  React.useEffect(() => {
    if (open) { setStep("template"); setSelectedTemplateId(null); setPreviewTemplate(null); setSubject(""); setBody(""); setError(null); setSending(false); loadEmailTemplates(); }
  ...
  const resolveBody = (raw: string) => {
    ...
    return raw.replace(/\{\{name\}\}/g, name).replace(/\{\{lead_name\}\}/g, name).replace(/\{\{lead_first_name\}\}/g, name.split(" ")[0]);
  };

  const handleNext = () => {
    const template = emailTemplates.find((t) => String(t.id) === selectedTemplateId);
    if (template) { setSubject(template.subject); setBody(stripHtml(resolveBody(template.body || ""))); }
    setStep("compose");
  };

  const handleNewTplSaved = (tpl: EmailTemplate) => {
    setEmailTemplates((prev) => [tpl, ...prev]); setSelectedTemplateId(String(tpl.id)); setNewTplOpen(false);
  };

  // ── SEND: calls onSent() before closing so parent can refresh immediately ──
  const handleSend = async () => {
    ...
    try {
      await LeadEmailAPI.sendNow({
        ...
        email_body: body.trim(), // plain text — backend stores as-is
        sender_email: lead.email ?? null,
      });
      onSent?.();   // ← notify parent BEFORE closing
      onClose();
    } catch (err: unknown) {
  ...
  const handleSaveAsDraft = async () => {
    ...
    try { await LeadEmailAPI.saveAsDraft({ lead: lead.id, subject: subject.trim(), email_body: body.trim(), sender_email: lead.email ?? null }); }
    catch { /* silent */ }
  };
  ...
  return (
    <>
      ...
      <Dialog open={open && !newTplOpen} onClose={handleClose} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: "16px", maxHeight: "90vh" } }}>
        ...
        {step === "template" && (
          <>
            ...
            <DialogContent sx={{ pt: 1, pb: 0 }}>
              <Box onClick={() => { setSelectedTemplateId(null); setSubject(""); setBody(""); setStep("compose"); }}
              ...
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.5 }}>
                ...
                <Button size="small" onClick={() => setNewTplOpen(true)} sx={{ textTransform: "none", fontSize: "12px", fontWeight: 600, color: "#1F2937", border: "1px solid #E5E7EB", borderRadius: "6px", px: 1.5, py: 0.5, "&:hover": { bgcolor: "#F3F4F6" } }}>+ New Template</Button>
              </Box>
                  ...
                  <Stack spacing={0} divider={<Divider />} sx={{ maxHeight: 340, overflowY: "auto" }}>
                    {emailTemplates.map((template) => (
                      <Box key={template.id} onClick={() => setSelectedTemplateId(String(template.id))}
                        ...
                        <FormControlLabel value={String(template.id)}
            ...
            <DialogContent sx={{ pt: 0, pb: 0 }}>
              ...
              <Box sx={{ border: "1px solid #E2E8F0", borderRadius: "12px", overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                ...
                <Box sx={{ bgcolor: "#F8FAFC", borderTop: "1px solid #E2E8F0", px: 2.5, py: 1.25, textAlign: "center" }}>
                  <Typography fontSize="11px" color="#94A3B8">Variables in <Box component="span" sx={{ color: "#7C3AED", fontWeight: 600 }}>purple</Box> will be auto-filled when sent</Typography>
                </Box>
              </Box>
            </DialogContent>
            <DialogActions sx={{ px: 3, pb: 3, pt: 2, gap: 1 }}>
              ...
              <Button variant="contained" onClick={() => { setSubject(previewTemplate.subject || ""); setBody(stripHtml(resolveBody(previewTemplate.body || ""))); setStep("compose"); }}
        ...
        {step === "compose" && (
          <>
            ...
            <DialogContent sx={{ pt: 0, pb: 0 }}>
              <Stack spacing={0} divider={<Divider />}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, py: 1 }}>
                  ...
                  <Chip label={recipientName} size="small" sx={{ bgcolor: "#EFF6FF", color: "#1D4ED8", fontWeight: 500, fontSize: "12px", height: 24 }} />
                ...
                <Box sx={{ py: 1.5 }}>
                  {error && <Alert severity="error" onClose={() => setError(null)} sx={{ borderRadius: "8px", mb: 1.5, fontSize: "13px" }}>{error}</Alert>}
                  <textarea ref={bodyRef} value={body} onChange={(e) => setBody(e.target.value)} onSelect={saveCursor} onKeyUp={saveCursor} onMouseUp={saveCursor}
                    placeholder="Write your message here..." disabled={sending}
                    style={{ width: "100%", minHeight: 260, border: "none", outline: "none", resize: "none", fontSize: "14px", color: "#374151", lineHeight: 1.75, fontFamily: "inherit", background: "transparent", padding: 0 }} />
                </Box>
              </Stack>
            </DialogContent>
            <Box sx={{ px: 2, py: 1, borderTop: "1px solid #F3F4F6", display: "flex", alignItems: "center", gap: 0.5 }}>
              {([
                { icon: <Typography fontWeight={700} fontSize="14px" sx={{ lineHeight: 1, fontFamily: "serif" }}>T</Typography>, title: "Bold", action: () => { saveCursor(); const { start, end } = cursorPos.current; setBody((p) => p.substring(0, start) + "**" + (p.substring(start, end) || "text") + "**" + p.substring(end)); } },
                { icon: <AttachFileIcon sx={{ fontSize: 18 }} />, title: "Attach", action: () => fileInputRef.current?.click() },
                { icon: <LinkIcon sx={{ fontSize: 18 }} />, title: "Link", action: () => { saveCursor(); const u = window.prompt("URL:", "https://"); if (u) insertAtCursor(`[link](${u})`); } },
                { icon: <EmojiEmotionsOutlinedIcon sx={{ fontSize: 18 }} />, title: "Emoji", action: () => { saveCursor(); const e = window.prompt("Emoji:", "😊"); if (e) insertAtCursor(e); } },
                ...
                { icon: <DataObjectIcon sx={{ fontSize: 18 }} />, title: "Variable", action: () => { saveCursor(); const v = window.prompt("Variable:", "lead_first_name"); if (v) insertAtCursor(`{{${v}}}`); } },
                { icon: <DriveFileRenameOutlineIcon sx={{ fontSize: 18 }} />, title: "Highlight", action: () => { saveCursor(); const { start, end } = cursorPos.current; setBody((p) => p.substring(0, start) + "==" + (p.substring(start, end) || "text") + "==" + p.substring(end)); } },
                ...
                <IconButton key={title} size="small" title={title} onClick={action} sx={{ color: "#6B7280", borderRadius: "6px", p: 0.6, "&:hover": { bgcolor: "#F3F4F6", color: "#111827" } }}>{icon}</IconButton>
              ))}
            </Box>
            <DialogActions sx={{ px: 3, pb: 3, pt: 1.5, borderTop: "1px solid #E5E7EB", gap: 1 }}>
              ...
              <Button onClick={handleSaveAsDraft} disabled={sending || !subject.trim() || !body.trim()} sx={{ height: 40, color: "#374151", fontWeight: 500, textTransform: "none", borderRadius: "8px", border: "1px solid #E5E7EB", px: 3, "&:hover": { bgcolor: "#F3F4F6" } }}>Save as Draft</Button>
...
export default function LeadDetailView() {
  ...
  // ── Email history lifted to parent so it survives dialog open/close ──
  ...
  React.useEffect(() => {
    if (!leads || leads.length === 0) dispatch(fetchLeads() as unknown as Parameters<typeof dispatch>[0]);
  }, [dispatch, leads]);
  ...
  const lead = React.useMemo((): LeadRecord | undefined => {
    ...
    return leads.find((l) => l.id.replace("#", "").replace("LN-", "").replace("LD-", "") === cleanId);
  }, [leads, id]);

  React.useEffect(() => {
    if (!id) return;
    LeadAPI.getById(decodeURIComponent(id)).then((data) => setFullLead(data as unknown as LeadRecord)).catch(() => setFullLead(null));
  }, [id]);
  ...
  const fetchNotes = React.useCallback(async (leadUuid: string) => {
    try {
      ...
      setNotes(results.filter((n) => !n.is_deleted).map((n) => ({ id: n.id, uuid: n.id, title: n.title ?? "", content: n.note ?? "", time: n.created_at ? formatNoteTime(n.created_at) : "" })));
    } catch (err: unknown) { setNotesError(err instanceof Error ? err.message : (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to load notes"); }
    finally { setNotesLoading(false); }
  }, []);

  const fetchDocuments = React.useCallback((leadDocs: DocumentEntry[]) => {
    setDocsLoading(true); setDocsError(null);
    try { setDocuments(Array.isArray(leadDocs) ? leadDocs.map(normalizeDocument) : []); }
  ...
  const fetchCallHistory = React.useCallback(async (leadUuid: string) => {
    ...
    } catch (err: unknown) { setCallHistoryError(err instanceof Error ? err.message : (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to load call history"); }
  ...
  const fetchSMSHistory = React.useCallback(async (leadUuid: string) => {
    ...
    } catch (err: unknown) { setSmsHistoryError(err instanceof Error ? err.message : (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to load SMS history"); }
    finally { setSmsHistoryLoading(false); }
  }, []);

  React.useEffect(() => { dispatch(fetchLeads() as unknown as Parameters<typeof dispatch>[0]); }, [location.key, dispatch]);
  ...
  React.useEffect(() => {
    if (activeLead) {
      ...
      fetchEmailHistory(activeLead.id); // load email history on mount too
    }
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      const { data: created } = await api.post("/leads/notes/", { title: newNoteTitle.trim() || "Note", note: newNoteContent.trim(), lead: decodeURIComponent(id || ""), is_active: true, is_deleted: false, ...(userId !== null && { created_by: userId }) });
      const createdNote = created as RawNote;
      setNotes((prev) => [...prev, { id: createdNote.id, uuid: createdNote.id, title: createdNote.title ?? newNoteTitle, content: createdNote.note ?? newNoteContent, time: createdNote.created_at ? formatNoteTime(createdNote.created_at) : "" }]);
      setNewNoteTitle(""); setNewNoteContent("");
    } catch (err: unknown) { setNotesError(err instanceof Error ? err.message : (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to save note"); }
  ...
  const handleSaveEdit = async (noteId: string) => {
    ...
    try {
      ...
      const userId = getCurrentUserId();
      const { data: updated } = await api.put(`/leads/notes/${noteId}/update/`, { title: editTitle.trim(), note: editContent.trim(), lead: decodeURIComponent(id || ""), ...(userId !== null && { created_by: userId }) });
      const updatedNote = updated as RawNote;
      setNotes((prev) => prev.map((n) => n.id === noteId ? { ...n, title: updatedNote.title ?? editTitle, content: updatedNote.note ?? editContent } : n));
      setEditingNoteId(null); setEditTitle(""); setEditContent("");
    } catch (err: unknown) { setNotesError(err instanceof Error ? err.message : (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to update note"); }
    finally { setEditSubmitting(false); }
  };

  const handleDeleteNote = async (noteId: string) => {
    try { await api.delete(`/leads/notes/${noteId}/delete/`); setNotes((prev) => prev.filter((n) => n.id !== noteId)); setDeleteNoteDialog(null); }
    catch (err: unknown) { setNotesError(err instanceof Error ? err.message : (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? "Failed to delete note"); }
  };
  ...
  const handleAddNextAction = async () => {
    ...
    try {
      ...
      await api.put(`/leads/${decodeURIComponent(id || "")}/update/`, { clinic_id: activeLead.clinic_id, department_id: activeLead.department_id, full_name: activeLead.full_name || activeLead.name, contact_no: activeLead.contact_no || activeLead.phone || activeLead.phone_number || "", source: activeLead.source || "Unknown", treatment_interest: activeLead.treatment_interest || "N/A", book_appointment: activeLead.book_appointment || false, appointment_date: activeLead.appointment_date || "", slot: activeLead.slot || "", is_active: activeLead.is_active !== false, partner_inquiry: activeLead.partner_inquiry || false, next_action_type: actionType, next_action_status: actionStatus, next_action_description: actionDescription.trim() });
      setOpenAddActionDialog(false); setActionType(""); setActionStatus("pending"); setActionDescription(""); setActionError(null);
  ...
  const closeActionDialog = () => { setOpenAddActionDialog(false); setActionType(""); setActionStatus("pending"); setActionDescription(""); setActionError(null); };
  ...
  const handleConvertLead = async () => {
    ...
    try {
      ...
      if (convertLead.rejected.match(result as Parameters<typeof convertLead.rejected.match>[0])) { setConvertError(String((result as { payload?: string; error?: { message?: string } })?.payload || (result as { error?: { message?: string } })?.error?.message || "Failed to convert lead.")); return; }
      setFullLead((prev) => ({ ...(prev ?? activeLead), lead_status: "converted", status: "converted" } as LeadRecord));
      try { const ids: string[] = JSON.parse(localStorage.getItem("converted_lead_ids") || "[]"); if (!ids.includes(leadUuid)) localStorage.setItem("converted_lead_ids", JSON.stringify([...ids, leadUuid])); } catch { /* no-op */ }
      setOpenConvertPopup(false);
    } catch (err: unknown) { setConvertError(err instanceof Error ? err.message : "Failed to convert lead."); }
    finally { setConvertLoading(false); }
  };

  const handleEdit = () => { if (!activeLead) return; navigate(`/leads/edit/${getCleanLeadId(activeLead.id)}`, { state: { lead: activeLead } }); };

  if (loading && !activeLead) return (<Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "400px" }}><Stack alignItems="center" spacing={2}><CircularProgress /><Typography color="text.secondary">Loading lead details...</Typography></Stack></Box>);
  if (error) return (<Box p={3}><Alert severity="error"><Typography fontWeight={600}>Failed to load lead</Typography><Typography variant="body2">{error}</Typography><Typography variant="body2" sx={{ mt: 1, color: "primary.main", cursor: "pointer", textDecoration: "underline" }} onClick={() => dispatch(fetchLeads() as unknown as Parameters<typeof dispatch>[0])}>Try again</Typography></Alert></Box>);
  if (!activeLead) return (<Box p={3}><Alert severity="warning"><Typography fontWeight={600}>Lead not found</Typography><Typography variant="body2">The lead you're looking for doesn't exist or may have been deleted.</Typography><Typography variant="body2" sx={{ mt: 1, color: "primary.main", cursor: "pointer", textDecoration: "underline" }} onClick={() => navigate("/leads")}>Go back to Leads Hub</Typography></Alert></Box>);
  ...
  const leadCreatedAt     = activeLead.created_at ? new Date(activeLead.created_at).toLocaleString("en-IN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "N/A";
  ...
  const treatmentInterest     = activeLead.treatment_interest ? activeLead.treatment_interest.split(",").map((t) => capitalizeWords(t.trim())) : [];
  ...
  const appointmentPersonnel  = hasAppointment ? capitalizeWords(activeLead.personal_name || activeLead.personnel || "N/A") : "N/A";
  const appointmentDate       = hasAppointment && activeLead.appointment_date ? new Date(activeLead.appointment_date).toLocaleDateString("en-GB") : "N/A";
  ...
  return (
    <Box p={1} minHeight="100vh">
      ...
      <Card elevation={0} sx={{ position: "relative", p: 5, mb: 3, borderRadius: "16px", backgroundColor: "#FAFAFA", overflow: "hidden", boxShadow: "none" }}>
        <Box component="img" src={Lead_Subtract} alt="" sx={{ position: "absolute", top: "6px", left: 0, width: "100%", height: "100%", objectFit: "fill", zIndex: 0, pointerEvents: "none" }} />
        <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ position: "relative", width: "100%", zIndex: 1 }}>
          <Stack direction="row" alignItems="flex-end" justifyContent="space-between" sx={{ width: "100%", pl: 2, pr: 1 }}>
            <Avatar sx={{ bgcolor: "#EEF2FF", color: "#6366F1", width: 45, height: 45, fontSize: "30px", fontWeight: 700, transform: "translateY(-35px)", ml: -2, flexShrink: 0 }}>{leadInitials}</Avatar>
            <Stack spacing={0.5} sx={{ flex: 1.3, transform: "translateY(14px)" }}><Typography variant="caption" color="text.secondary" fontSize="10px">Lead Name</Typography><Typography fontWeight={700} variant="body1" fontSize="12px">{leadName}</Typography></Stack>
            <Stack spacing={0.5} sx={{ flex: 1.3, transform: "translateY(14px)" }}><Typography variant="caption" color="text.secondary" fontSize="10px">Lead ID</Typography><Typography fontWeight={600} variant="body1" fontSize="12px">{leadDisplayId}</Typography></Stack>
            <Stack spacing={0.5} sx={{ flex: 1.3, transform: "translateY(14px)" }}><Typography variant="caption" color="text.secondary" fontSize="10px">Source</Typography><Stack direction="row" alignItems="center" spacing={0.5}><Box sx={{ width: 16, height: 16, bgcolor: "#FFB800", borderRadius: "50%" }} /><Typography fontWeight={600} variant="body1" fontSize="12px">{leadSource}</Typography></Stack></Stack>
            <Stack spacing={0.5} sx={{ flex: 1.3, transform: "translateY(14px)" }}><Typography variant="caption" color="text.secondary" fontSize="10px">Lead Status</Typography><Chip label={isConverted ? "Converted" : leadStatus} size="small" sx={isConverted ? pillChipSx("#16A34A", "rgba(22,163,74,0.10)") : pillChipSx("#5B8FF9", "rgba(91,143,249,0.10)")} /></Stack>
            <Stack spacing={0.5} sx={{ flex: 1.3, transform: "translateY(14px)" }}><Typography variant="caption" color="text.secondary" fontSize="10px">Lead Quality</Typography><Chip label={leadQuality} size="small" sx={leadQuality === "Hot" ? pillChipSx("#FF4D4F", "rgba(255,77,79,0.10)") : leadQuality === "Warm" ? pillChipSx("#FFC53D", "rgba(255,197,61,0.10)") : pillChipSx("#52C41A", "rgba(82,196,26,0.10)")} /></Stack>
            <Stack spacing={0.5} sx={{ flex: 1.3, transform: "translateY(14px)" }}><Typography variant="caption" color="text.secondary" fontSize="10px">AI Lead Score</Typography><Typography fontWeight={700} color="#EC4899" fontSize="12px">{leadScore}</Typography></Stack>
          </Stack>
        </Stack>
      </Card>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        <Box sx={{ bgcolor: "#FAFAFA", borderRadius: "10px", p: 0.8, width: "fit-content", display: "inline-flex", alignItems: "center" }}>
          <Stack direction="row" spacing={1}>
            {["Patient Info", "History", "Next Action"].map((tab) => {
              ...
              return (
                <Box key={tab} onClick={() => setActiveTab(tab)} sx={{ cursor: "pointer", px: 2.5, py: 1, borderRadius: "8px", bgcolor: sel ? "#FFFFFF" : "transparent", color: sel ? "#E17E61" : "#232323", boxShadow: sel ? "0 2px 6px rgba(0,0,0,0.08)" : "none", transition: "all 0.2s ease", display: "flex", alignItems: "center" }}>
        ...
        <Stack direction="row" spacing={2}>
          <Button variant="outlined" startIcon={<EditOutlinedIcon />} onClick={handleEdit} sx={{ borderRadius: "8px", textTransform: "none", bgcolor: "#f3f3f3", color: "#505050", border: "none", boxShadow: "none", "&:hover": { bgcolor: "#f3f3f3", color: "#232323", border: "none", boxShadow: "none" } }}>Edit</Button>
          <Button variant="contained" onClick={handleOpenPopup} startIcon={<SwapHorizIcon />} disabled={isConverted} sx={{ borderRadius: "8px", textTransform: "none", bgcolor: isConverted ? "#E2E8F0" : "#505050", color: isConverted ? "#94A3B8" : "#FFFFFF", px: 2, boxShadow: "none", "&:hover": { bgcolor: isConverted ? "#E2E8F0" : "#232323", boxShadow: "none" }, "&:disabled": { bgcolor: "#E2E8F0", color: "#94A3B8" } }}>
      ...
      {activeTab === "Patient Info" && (
        <PatientInfoTab lead={activeLead} leadPhone={leadPhone} leadEmail={leadEmail} leadLocation={leadLocation} leadGender={leadGender} leadAge={leadAge} leadMaritalStatus={leadMaritalStatus} leadAddress={leadAddress} leadLanguage={leadLanguage} leadAssigned={leadAssigned} leadCreatedAt={leadCreatedAt} partnerName={partnerName} partnerAge={partnerAge} partnerGender={partnerGender} leadSubSource={leadSubSource} leadCampaignName={leadCampaignName} leadCampaignDuration={leadCampaignDuration} hasAppointment={hasAppointment} appointmentDepartment={appointmentDepartment} appointmentPersonnel={appointmentPersonnel} appointmentDate={appointmentDate} appointmentSlot={appointmentSlot} appointmentRemark={appointmentRemark} treatmentInterest={treatmentInterest} documents={documents} docsLoading={docsLoading} docsError={docsError} onClearDocsError={() => setDocsError(null)} />
      )}
      ...
      {activeTab === "History" && (
        <HistoryTab
          ...
          leadName={leadName} leadPhone={leadPhone} leadEmail={leadEmail} leadAssigned={leadAssigned} leadCreatedAt={leadCreatedAt}
          appointmentDate={appointmentDate} appointmentSlot={appointmentSlot} appointmentDepartment={appointmentDepartment} appointmentPersonnel={appointmentPersonnel} appointmentRemark={appointmentRemark}
          treatmentInterest={treatmentInterest} hasAppointment={hasAppointment}
          callHistory={callHistory} callHistoryLoading={callHistoryLoading} callHistoryError={callHistoryError} onRefreshCallHistory={() => fetchCallHistory(activeLead.id)}
          smsHistory={smsHistory} smsHistoryLoading={smsHistoryLoading} smsHistoryError={smsHistoryError} onRefreshSmsHistory={() => fetchSMSHistory(activeLead.id)}
          // ── email history passed from parent ──
      ...
      {activeTab === "Next Action" && (
        <NextActionTab lead={activeLead} nextActionType={nextActionType} nextActionStatus={nextActionStatus} nextActionDescription={nextActionDescription} isAppointment={isAppointment} isFollowUp={isFollowUp} availableActions={availableActions} openAddActionDialog={openAddActionDialog} setOpenAddActionDialog={setOpenAddActionDialog} actionType={actionType} setActionType={setActionType} actionStatus={actionStatus} setActionStatus={setActionStatus} actionDescription={actionDescription} setActionDescription={setActionDescription} actionSubmitting={actionSubmitting} actionError={actionError} setActionError={setActionError} onAddNextAction={handleAddNextAction} onCloseActionDialog={closeActionDialog} notes={notes} notesLoading={notesLoading} notesError={notesError} setNotesError={setNotesError} editingNoteId={editingNoteId} editTitle={editTitle} setEditTitle={setEditTitle} editContent={editContent} setEditContent={setEditContent} editSubmitting={editSubmitting} newNoteTitle={newNoteTitle} setNewNoteTitle={setNewNoteTitle} newNoteContent={newNoteContent} setNewNoteContent={setNewNoteContent} noteSubmitting={noteSubmitting} deleteNoteDialog={deleteNoteDialog} setDeleteNoteDialog={setDeleteNoteDialog} onStartEditNote={handleStartEdit} onCancelEditNote={handleCancelEdit} onSaveEditNote={handleSaveEdit} onAddNote={handleAddNote} onDeleteNote={handleDeleteNote} />
      )}

      {/* ── CONVERT POPUP ── */}
      <Dialog open={openConvertPopup} onClose={convertLoading ? undefined : handleClosePopup} PaperProps={{ sx: { borderRadius: "24px", p: 4, textAlign: "center", maxWidth: "420px", boxShadow: "0px 20px 25px -5px rgba(0,0,0,0.1)" } }}>
        <DialogContent sx={{ p: 0 }}>
          <Stack alignItems="center" spacing={2.5}>
            <Box sx={{ width: 64, height: 64, borderRadius: "50%", bgcolor: "#FFF7ED", display: "flex", alignItems: "center", justifyContent: "center" }}><SwapHorizIcon sx={{ fontSize: 32, color: "#F97316" }} /></Box>
            <Box>
              <Typography variant="h6" fontWeight={700} gutterBottom>Convert Lead</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ px: 2, lineHeight: 1.6 }}>Are you sure you want to Convert <b>"{leadName}"</b> lead into a patient &amp; register it?</Typography>
            </Box>
      ...
      {/* ── Email Dialog ── onSent switches to History > Email and fetches immediately ── */}
      <EmailDialog
