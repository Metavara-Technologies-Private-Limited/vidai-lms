...
export default function LeadDetailView() {
      ...
      <Box p={3}>
        <Alert severity="error">
          ...
          <Typography variant="body2" sx={{ mt: 1, color: "primary.main", cursor: "pointer", textDecoration: "underline" }}
            onClick={() => dispatch(fetchLeads() as any)}>Try again</Typography>
        </Alert>
      </Box>
    );
  }
  ...
  const leadInitials = leadName.split(" ").map((w: string) => w[0]).join("").toUpperCase().slice(0, 2);
  ...
  const appointmentDate = lead.appointment_date
    ? new Date(lead.appointment_date).toLocaleDateString("en-GB") : "N/A";
  ...
  return (
    <Box sx={{ p: 3, minHeight: "100vh" }}>
      ...
      {/* ══════════════════════════════════════
          TOP SUMMARY — white card, avatar overlaps top edge
      ══════════════════════════════════════ */}
      <Box
        sx={{
          position: "relative",
          ...
          pt: "44px",   // space for avatar to overlap from top
          pb: 2,
          px: 3,
          pl: "108px",  // space for avatar on left
        }}
      >
        {/* Avatar — sits at top-left, half outside the card */}
        <Box
          sx={{
            position: "absolute",
            top: -28,
      ...
      {/* ══════════════════════════════════════
          TABS + ACTION BUTTONS — white card row
      ══════════════════════════════════════ */}
      <Box sx={{ bgcolor: "#FFFFFF", borderRadius: "14px", border: "1px solid #E8EDF2", px: 2.5, py: 1.5, mb: 2 }}>
          ...
          <Stack direction="row" spacing={1}>
            {tabConfig.map((t) => {
              ...
              return (
                ...
                >
                  <Box sx={{ color: isActive ? "#EA580C" : "#64748B", display: "flex", alignItems: "center" }}>
                    {t.icon}
                  </Box>
                  <Typography
                    fontSize="13px"
      ...
      {activeTab === "Patient Info" && (
        <Stack direction="row" spacing={2.5} alignItems="flex-start">
          ...
          {/* LEFT — Single white outer card with 3 gray section blocks inside */}
          <Box sx={{ flex: 2, bgcolor: "#FFFFFF", borderRadius: "16px", border: "1px solid #E8EDF2", p: 3 }}>
            <Typography fontWeight={700} fontSize="15px" color="#0F172A" mb={2.5}>Basic Information</Typography>

            <Stack spacing={2}>
              {/* Section 1 — Lead Information */}
              <Box sx={{ bgcolor: "#F8FAFC", borderRadius: "12px", p: 2.5 }}>
              ...
              {/* Section 3 — Source & Campaign Details */}
              <Box sx={{ bgcolor: "#F8FAFC", borderRadius: "12px", p: 2.5 }}>
          ...
          <Stack spacing={2} sx={{ flex: 1 }}>
            {/* Appointment */}
            <Box sx={{ bgcolor: "#FFFFFF", borderRadius: "16px", border: "1px solid #E8EDF2", overflow: "hidden" }}>
              <Box sx={{ px: 3, py: 2, bgcolor: "#F0FDF4", borderBottom: "1px solid #DCFCE7" }}>
              ...
              <Box sx={{ p: 2.5 }}>
                <Stack direction="row" justifyContent="space-between" mb={2}>
                  <Box>
                    ...
                    <Typography fontWeight={600} fontSize="13px" color="#0F172A">{appointmentDepartment}</Typography>
                  </Box>
                  <Box>
                    <FieldLabel>PERSONNEL</FieldLabel>
                    <Typography fontWeight={600} fontSize="13px" color="#0F172A">{appointmentPersonnel}</Typography>
                  </Box>
                </Stack>
                <Stack direction="row" justifyContent="space-between" mb={2}>
                  <Box>
                    <FieldLabel>DATE</FieldLabel>
                    <Typography fontWeight={600} fontSize="13px" color="#0F172A">{appointmentDate}</Typography>
                  </Box>
                  <Box>
                    <FieldLabel>SLOT</FieldLabel>
                    <Typography fontWeight={600} fontSize="13px" color="#0F172A">{appointmentSlot}</Typography>
                  </Box>
                </Stack>
            ...
            {/* Treatment Interest */}
            <Box sx={{ bgcolor: "#FFFFFF", borderRadius: "16px", border: "1px solid #E8EDF2", p: 2.5 }}>
                ...
                {treatmentInterest.length > 0 ? (
                  treatmentInterest.map((t: string, i: number) => (
                    <Chip key={i} label={t} size="small"
                      sx={{ bgcolor: "#F5F3FF", color: "#7C3AED", fontWeight: 600, fontSize: "12px", borderRadius: "6px", mb: 1, height: 26 }} />
                  ))
                ) : (
      ...
      {activeTab === "History" && (
        <Stack direction="row" spacing={2.5}>
          {/* Activity Timeline */}
          <Box sx={{ width: 340, flexShrink: 0, bgcolor: "#F8FAFC", borderRadius: "16px", border: "1px solid #EEF2F7", p: 3 }}>
            <Typography fontWeight={700} fontSize="15px" color="#0F172A" mb={3}>Activity Timeline</Typography>
            <Stack spacing={0}>
              <TimelineItem icon={<ChatBubbleOutlineIcon sx={{ fontSize: 14, color: "#6366F1" }} />}
                title="Appointment Booked - Confirmation sent (SMS)" time={leadCreatedAt} />
              <TimelineItem icon={<CallOutlinedIcon sx={{ fontSize: 14, color: "#10B981" }} />}
                title="Outgoing call attempted - Connected" time={leadCreatedAt}
                onClick={() => setHistoryView("call")} isClickable />
              <TimelineItem icon={<EmailOutlinedIcon sx={{ fontSize: 14, color: "#F59E0B" }} />}
                title="Patient shared contact number and email" time={leadCreatedAt}
                onClick={() => setHistoryView("email")} isClickable />
              <TimelineItem icon={<EmailOutlinedIcon sx={{ fontSize: 14, color: "#3B82F6" }} />}
                title="Sent an Welcome Email" time={leadCreatedAt}
                onClick={() => setHistoryView("email")} isClickable />
              <TimelineItem isAvatar avatarInitial={leadAssigned.charAt(0)}
                title={`Assigned to ${leadAssigned}`} time={leadCreatedAt} />
              <TimelineItem icon={<ChatBubbleOutlineIcon sx={{ fontSize: 14, color: "#8B5CF6" }} />}
          ...
          {/* Chat Panel */}
          <Box sx={{
            flex: 1, bgcolor: "#F8FAFC", borderRadius: "16px", border: "1px solid #EEF2F7",
            display: "flex", flexDirection: "column", overflow: "hidden", maxHeight: "600px",
          }}>
            {historyView === "chatbot" && (
              <>
              ...
              <>
                <Box sx={{ px: 3, py: 2, borderBottom: "1px solid #F1F5F9" }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    ...
                    <IconButton onClick={() => (window.location.href = `mailto:${leadEmail}`)}
                      sx={{ bgcolor: "#EFF6FF", width: 32, height: 32, "&:hover": { bgcolor: "#DBEAFE" } }}>
                    ...
                    <Box sx={{ bgcolor: "#FFFFFF", borderRadius: "12px", border: "1px solid #E2E8F0", p: 2.5 }}>
                      <Stack direction="row" justifyContent="space-between" mb={2}>
                        <Stack direction="row" spacing={1.5} alignItems="center">
                          <Avatar sx={{ width: 34, height: 34, bgcolor: "#EEF2FF", color: "#6366F1", fontSize: "12px", fontWeight: 700 }}>
                            {leadInitials}
                          </Avatar>
                          <Box>
      ...
      {activeTab === "Next Action" && (
        <Stack direction="row" spacing={2.5} alignItems="flex-start">
          ...
          <Box sx={{ width: 310, flexShrink: 0, bgcolor: "#F8FAFC", borderRadius: "16px", border: "1px solid #EEF2F7", overflow: "hidden" }}>
            <Stack direction="row" justifyContent="space-between" alignItems="center"
              sx={{ px: 2.5, py: 2, borderBottom: "1px solid #EEF2F7" }}>
            ...
            <Box sx={{ p: 2 }}>
              ...
              >
                ...
                <Typography fontSize="13px" fontWeight={600} color="#1E1B4B" mb={0.5}>
                  Book Appointment{" "}
                  <Typography component="span" fontSize="11px" color="#6B7280" fontWeight={400}>
              ...
              <Typography variant="caption" fontWeight={700} color="#94A3B8"
                sx={{ textTransform: "uppercase", letterSpacing: "0.8px", display: "block", mb: 1.5, fontSize: "10px" }}>
                Next Action
              </Typography>
              ...
              <Box sx={{ border: "1px solid #E2E8F0", borderRadius: "12px", p: 2, mb: 2.5, bgcolor: "#FFFFFF" }}>
                <Stack direction="row" spacing={1.5} alignItems="flex-start">
                  <Box sx={{ p: 0.75, bgcolor: "#EFF6FF", borderRadius: "8px", mt: 0.25, flexShrink: 0 }}>
                    <CallOutlinedIcon sx={{ color: "#3B82F6", fontSize: 17 }} />
                  </Box>
                  <Box sx={{ flexGrow: 1 }}>
                    ...
                    <Stack direction="row" spacing={3} mb={1.5}>
                      <Box>
                        <Typography variant="caption" color="#94A3B8" fontWeight={600}
                          sx={{ textTransform: "uppercase", fontSize: "9px", letterSpacing: "0.5px", display: "block", mb: 0.25 }}>DUE</Typography>
                        <Typography fontSize="13px" fontWeight={600}>Today</Typography>
                      </Box>
                      <Box>
                        <Typography variant="caption" color="#94A3B8" fontWeight={600}
                          sx={{ textTransform: "uppercase", fontSize: "9px", letterSpacing: "0.5px", display: "block", mb: 0.25 }}>STATUS</Typography>
                        <Chip label={nextActionStatus} size="small"
                          sx={{ bgcolor: "#EFF6FF", color: "#3B82F6", fontWeight: 600, borderRadius: "6px", height: 20, fontSize: "11px" }} />
                      </Box>
                    </Stack>
                    <Box mb={2}>
                      <Typography variant="caption" color="#94A3B8" fontWeight={600}
                        sx={{ textTransform: "uppercase", fontSize: "9px", letterSpacing: "0.5px", display: "block", mb: 0.25 }}>DESCRIPTION</Typography>
                    ...
                    <Stack direction="row" spacing={1}>
                      <Button variant="outlined" size="small"
                        ...
                        sx={{
                          ...
                          borderColor: "#E2E8F0", color: "#475569", fontSize: "12px", py: 0.4,
                          "&:hover": { borderColor: "#CBD5E1", bgcolor: "#F8FAFC" },
                        }}>
                        Mark Done
                      </Button>
                      <Button variant="contained" size="small"
                        startIcon={<CallOutlinedIcon sx={{ fontSize: 13 }} />}
                        sx={{
                          textTransform: "none", borderRadius: "8px",
                          bgcolor: "#1E293B", color: "#FFFFFF", fontSize: "12px", py: 0.4,
                          boxShadow: "none",
                          "&:hover": { bgcolor: "#0F172A", boxShadow: "none" },
                        }}>
                        Call Now
              ...
              <Box sx={{ border: "1px solid #E2E8F0", borderRadius: "12px", p: 2, bgcolor: "#FFFFFF" }}>
                <Stack direction="row" spacing={1.5} alignItems="flex-start">
                  <Box sx={{ p: 0.75, bgcolor: "#F0FDF4", borderRadius: "8px", mt: 0.25, flexShrink: 0 }}>
                    <CallOutlinedIcon sx={{ color: "#10B981", fontSize: 17 }} />
                  </Box>
                  <Box sx={{ flexGrow: 1 }}>
                    ...
                    <Stack direction="row" spacing={3}>
                      ...
                      <Box>
                        ...
                        <Chip label="Completed" size="small"
                          sx={{ bgcolor: "#F0FDF4", color: "#16A34A", fontWeight: 600, borderRadius: "6px", height: 20, fontSize: "11px" }} />
                      </Box>
                    </Stack>
                  </Box>
                </Stack>
          ...
          {/* RIGHT — Notes Panel */}
          <Box sx={{ flexGrow: 1, bgcolor: "#F8FAFC", borderRadius: "16px", border: "1px solid #EEF2F7", overflow: "hidden" }}>
            <Box sx={{ px: 2.5, py: 2, borderBottom: "1px solid #EEF2F7", bgcolor: "#F8FAFC" }}>
            ...
            <Box sx={{ p: 2.5, bgcolor: "#F8FAFC" }}>
                    ...
                    editingNoteId === note.id ? (
                      <Box key={note.id} sx={{ p: 3, borderRadius: "12px", border: "2px solid #6366F1", bgcolor: "#FFFFFF" }}>
                        ...
                        <TextField fullWidth multiline minRows={3} value={editContent}
                          onChange={(e) => setEditContent(e.target.value)} variant="standard" placeholder="Note content..."
                        ...
                        <Stack direction="row" spacing={1} justifyContent="flex-end">
                          <Button size="small" onClick={handleCancelEdit} disabled={editSubmitting}
                            sx={{ textTransform: "none", fontSize: "13px", color: "#64748B", borderRadius: "8px" }}>Cancel</Button>
              ...
              <Box sx={{ borderRadius: "12px", border: "1px solid #E2E8F0", overflow: "hidden", bgcolor: "#FFFFFF" }}>
                <TextField fullWidth placeholder="Title" value={newNoteTitle} onChange={(e) => setNewNoteTitle(e.target.value)}
                  variant="standard"
                ...
                <Stack direction="row" alignItems="flex-end">
                  <TextField fullWidth multiline minRows={2} placeholder="Write note here..." value={newNoteContent}
          ...
          <Stack alignItems="center" spacing={2.5}>
            ...
            <Stack direction="row" spacing={2} sx={{ width: "100%", mt: 2 }}>
              <Button fullWidth onClick={handleClosePopup} variant="outlined"
                sx={{ borderRadius: "12px", textTransform: "none", fontWeight: 600, color: "#475569", borderColor: "#E2E8F0", py: 1.2 }}>Cancel</Button>
              <Button fullWidth variant="contained" onClick={handleClosePopup}
                sx={{ bgcolor: "#475569", borderRadius: "12px", textTransform: "none", fontWeight: 600, py: 1.2, boxShadow: "none", "&:hover": { bgcolor: "#334155" } }}>Convert</Button>
            </Stack>
          </Stack>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={openAddActionDialog} onClose={() => setOpenAddActionDialog(false)}
        ...
        <DialogContent sx={{ p: 0 }}>
          <Stack spacing={3}>
            ...
            <Box>
              <Typography fontSize="13px" fontWeight={600} mb={1}>Action Type *</Typography>
              <TextField select fullWidth size="small" value={actionType} onChange={(e) => setActionType(e.target.value)}
            ...
            <Box>
              ...
              <TextField select fullWidth size="small" value={actionStatus} onChange={(e) => setActionStatus(e.target.value)}
                sx={{ "& .MuiOutlinedInput-root": { borderRadius: "8px" } }}>
            ...
            <Box>
              ...
              <TextField fullWidth multiline rows={3} size="small" value={actionDescription}
                onChange={(e) => setActionDescription(e.target.value)} placeholder="Enter action description..."
                sx={{ "& .MuiOutlinedInput-root": { borderRadius: "8px" } }} />
            </Box>
            <Stack direction="row" spacing={2} justifyContent="flex-end">
              <Button onClick={() => { setOpenAddActionDialog(false); setActionType(""); setActionStatus("pending"); setActionDescription(""); }}
                variant="outlined" disabled={actionSubmitting}
                sx={{ borderRadius: "8px", textTransform: "none", fontWeight: 600, color: "#475569", borderColor: "#E2E8F0" }}>Cancel</Button>
              <Button onClick={handleAddNextAction} variant="contained"
                disabled={!actionType || !actionDescription || actionSubmitting}
                sx={{ bgcolor: "#334155", borderRadius: "8px", textTransform: "none", fontWeight: 600, boxShadow: "none", "&:hover": { bgcolor: "#1E293B" } }}>
      ...
      <Dialog open={deleteNoteDialog !== null} onClose={() => setDeleteNoteDialog(null)}
        ...
        <DialogContent sx={{ p: 0 }}>
          <Stack alignItems="center" spacing={2.5}>
            ...
            <Stack direction="row" spacing={2} sx={{ width: "100%", mt: 2 }}>
              <Button fullWidth onClick={() => setDeleteNoteDialog(null)} variant="outlined"
                sx={{ borderRadius: "12px", textTransform: "none", fontWeight: 600, color: "#475569", borderColor: "#E2E8F0", py: 1.2 }}>Cancel</Button>
  ...
  <Stack direction="row" spacing={2}>
    <Stack alignItems="center">
      <Box sx={{ width: 28, height: 28, borderRadius: "50%", bgcolor: "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        {isAvatar
          ? <Avatar sx={{ width: 16, height: 16, fontSize: "8px", bgcolor: "#6366F1" }}>{avatarInitial}</Avatar>
          : icon}
      </Box>
...
const ChatBubble = ({ side, text, time }: any) => (
  <Box sx={{ alignSelf: side === "left" ? "flex-start" : "flex-end", maxWidth: "68%", width: "fit-content" }}>
    <Box sx={{
      ...
      bgcolor: side === "left" ? "#FFFFFF" : "#1E293B",
      color: side === "left" ? "#0F172A" : "#FFFFFF",
  ...
  <Box sx={{ minWidth: "150px" }}>
    ...
    {isAvatar ? (
      <Stack direction="row" spacing={1} alignItems="center">
        <Avatar sx={{ width: 16, height: 16, fontSize: "8px", bgcolor: "#6366F1", color: "white" }}>
          {value?.charAt(0) || "U"}
        </Avatar>
  ...
  <Stack direction="row" justifyContent="space-between" alignItems="center">
    <Stack direction="row" spacing={1.5} alignItems="center">
      <Box sx={{
        ...
        bgcolor: name.endsWith(".pdf") ? "#FEF2F2" : "#EFF6FF",
