...
// ── Shared field style matching the screenshot (outlined, rounded, float label) ──
const fieldSx = {
  ...
  <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: "24px", overflow: "hidden" } }}>
    ...
    <DialogContent sx={{ p: 3 }}>
      <Box sx={{ mb: 3 }}>
        ...
        <Chip label={selectedLead?.full_name || selectedLead?.name} size="small" sx={{ bgcolor: "#EEF2FF", color: "#6366F1", fontWeight: 600, borderRadius: "8px", height: 32, fontSize: "0.875rem" }} />
      </Box>
      <Box>
        <Typography variant="body2" fontWeight={600} color="#475569" sx={{ mb: 1 }}>Message:</Typography>
        <TextField fullWidth multiline rows={6} placeholder="Type your message here..." value={smsMessage} onChange={(e) => setSmsMessage(e.target.value)}
        ...
        />
        <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1, textAlign: "right" }}>{smsMessage.length} / 160 characters</Typography>
      </Box>
    </DialogContent>
...
export const MailModal: React.FC<MailModalProps> = ({
  open, selectedLead, mailStep, selectedTemplate, setSelectedTemplate,
  onClose, onNextToCompose, onSaveAsTemplate, onBackToTemplates,
}) => (
  <Dialog open={open} onClose={onClose} fullWidth maxWidth={mailStep === 1 ? "sm" : "md"} PaperProps={{ sx: { borderRadius: "24px", overflow: "hidden" } }}>
    {mailStep === 1 ? (
      <>
        <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", p: 3 }}>
        ...
        <DialogContent sx={{ p: 0 }}>
          <Box onClick={onNextToCompose} sx={{ p: 3, textAlign: "center", borderBottom: "1px solid #F1F5F9", cursor: "pointer", "&:hover": { bgcolor: "#F8FAFC" } }}>
          ...
          <Box sx={{ p: 3 }}>
            ...
            <RadioGroup value={selectedTemplate} onChange={(e) => setSelectedTemplate(e.target.value)}>
              {EMAIL_TEMPLATES.map((tmpl) => (
                <Box key={tmpl.id} onClick={() => setSelectedTemplate(tmpl.title)} sx={{ display: "flex", alignItems: "flex-start", p: 2, mb: 1.5, border: "1px solid", borderColor: selectedTemplate === tmpl.title ? "#6366F1" : "#E2E8F0", borderRadius: "12px", cursor: "pointer", bgcolor: selectedTemplate === tmpl.title ? "#F5F7FF" : "transparent" }}>
                  <Radio size="small" value={tmpl.title} sx={{ mt: -0.5 }} />
                  <Box sx={{ flex: 1 }}>
                    <Typography variant="body2" fontWeight={700} color="#1E293B">{tmpl.title}</Typography>
        ...
        <DialogActions sx={{ p: 3, borderTop: "1px solid #F1F5F9" }}>
          <Button onClick={onClose} sx={{ color: "#64748B", textTransform: "none", fontWeight: 700 }}>Cancel</Button>
          <Button variant="contained" onClick={onNextToCompose} sx={{ bgcolor: "#334155", borderRadius: "10px", px: 4, textTransform: "none", fontWeight: 700, "&:hover": { bgcolor: "#1e293b" } }}>Next</Button>
        </DialogActions>
      </>
    ) : (
      <>
        ...
        <DialogContent sx={{ px: 3, py: 0 }}>
          <Stack spacing={0.5}>
            ...
            <Box sx={{ py: 3, minHeight: 320, overflowY: "auto" }}>
              ...
              <Typography variant="body2" sx={{ mb: 2 }}>Thank you for reaching out to <strong>Crysta IVF, Bangalore</strong>.</Typography>
            </Box>
          </Stack>
        </DialogContent>
        <DialogActions sx={{ p: 2, justifyContent: "space-between", bgcolor: "#F8FAFC", borderTop: "1px solid #F1F5F9" }}>
          <Stack direction="row" spacing={0.5}>
            {[TextFormatIcon, AttachFileIcon, InsertLinkIcon, InsertEmoticonIcon, InsertPhotoIcon, HistoryIcon, AddCircleOutlineIcon].map((Icon, i) => (
              <IconButton key={i} size="small"><Icon sx={{ fontSize: 20 }} /></IconButton>
            ))}
          </Stack>
          <Stack direction="row" spacing={1.5}>
            <Button onClick={onBackToTemplates} sx={{ color: "#64748B", textTransform: "none", fontWeight: 700 }}>Cancel</Button>
            <Button variant="outlined" onClick={onSaveAsTemplate} sx={{ borderColor: "#E2E8F0", color: "#1E293B", textTransform: "none", borderRadius: "10px", fontWeight: 700 }}>Save as Template</Button>
            <Button variant="contained" onClick={onClose} endIcon={<SendIcon sx={{ fontSize: 16 }} />} sx={{ bgcolor: "#334155", borderRadius: "10px", px: 3, textTransform: "none", fontWeight: 700, "&:hover": { bgcolor: "#1e293b" } }}>Send</Button>
          </Stack>
        </DialogActions>
      </>
    )}
...
export const BookAppointmentModal: React.FC<BookAppointmentModalProps> = ({
  ...
  >
    ...
    <DialogTitle sx={{ px: 3, pt: 3, pb: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <Typography fontWeight={800} fontSize="1.2rem" color="#111827">
        Book an Appointment
      </Typography>
      <IconButton
        onClick={onClose}
    ...
    <DialogContent sx={{ px: 3, pt: 2.5, pb: 1 }}>
      ...
      {/* Section label — matches screenshot "APPOINTMENT DETAILS" */}
      ...
      <Stack spacing={2.5}>
        ...
        {/* ── Row 1: Department (medical only) + Personnel ── */}
        <Box sx={{ display: "grid", gridTemplateColumns: IS_MEDICAL_APP ? "1fr 1fr" : "1fr", gap: 2 }}>

          {/* Department — only rendered when IS_MEDICAL_APP is true */}
          {IS_MEDICAL_APP && (
            <FormControl size="small" fullWidth sx={fieldSx}>
              ...
              >
                ...
                {appointment.departments.map((d) => (
                  <MenuItem key={d.id} value={d.id} sx={{ fontSize: "0.9rem" }}>{d.name}</MenuItem>
                ))}
              ...
              {IS_MEDICAL_APP && !appointment.selectedDepartmentId ? (
                ...
                [
                  <MenuItem key="__empty__" value="">
                    <Typography color="text.secondary" sx={{ fontSize: "0.85rem" }}>Select Personnel</Typography>
                  </MenuItem>,
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
          <DatePicker
            ...
            onChange={(v) => setDate(v ? new Date(v as unknown as string) : null)}
            minDate={new Date()}
            ...
            slotProps={{
              ...
              popper: {
                sx: {
                  "& .MuiPaper-root": { borderRadius: "14px", boxShadow: "0 8px 30px rgba(0,0,0,0.12)" },
                  "& .MuiPickersDay-root.Mui-selected": { bgcolor: "#334155" },
                  "& .MuiPickersDay-root.Mui-selected:hover": { bgcolor: "#1E293B" },
                },
              },
            }}
          />
          ...
          <FormControl size="small" fullWidth sx={fieldSx}>
            ...
            >
              <MenuItem value="" disabled>
                <Typography color="text.secondary" sx={{ fontSize: "0.85rem" }}>Select Time Slot</Typography>
              </MenuItem>
              {TIME_SLOTS.map((s) => (
