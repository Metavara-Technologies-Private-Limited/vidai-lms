...
// Build a LeadGeneratedByObject from an AssigneeOption
...
const isBackendNextActionType = (value: string): boolean => {
  ...
  return TASK_TYPES.some(
    (taskType) => taskType.toLowerCase().trim() === normalized,
  );
};
...
export default function AddNewLead() {
  ...
  // Lead Generated By stored as a full structured object
  ...
  // ── Load pipeline next action options ─────────────────────────────────────
  React.useEffect(() => {
  ...
  const handleNextTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setForm((prev) => ({ ...prev, nextType: e.target.value }));
  };
  ...
  const buildPayload = (): LeadPayload => {
    ...
    const stageAwareNextActionDescription =
      backendSafeNextType != null
        ...
        : [
            selectedNextType ? `Stage: ${selectedNextType}` : "",
            selectedNextDescription,
          ]
            .filter(Boolean)
            .join(" | ");

    const resolvedNextActionStatus =
      (form.nextStatus ?? "").trim() !== ""
        ? (form.nextStatus as LeadPayload["next_action_status"])
        : null;
    ...
    return {
      ...
      // lead_generated_by is the full name when set, otherwise undefined
      lead_generated_by: selectedLeadGeneratedBy ? `${selectedLeadGeneratedBy.first_name} ${selectedLeadGeneratedBy.last_name}`.trim() : undefined,
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const leadGeneratedByObj = selectedLeadGeneratedBy;
      ...
      if (recipientEmail) {
        ...
        const emailBody = [
          ...
          `- Lead Generated By (First Name): ${leadGeneratedByObj?.first_name || "-"}`,
          `- Lead Generated By (Last Name): ${leadGeneratedByObj?.last_name || "-"}`,
          `- Lead Generated By (Role): ${leadGeneratedByObj?.role || "-"}`,
          `- Lead Generated By (Email): ${leadGeneratedByObj?.email || "-"}`,
      ...
      if (payload.referral_department_id) {
        try {
          ...
          if (deptId) {
            dispatch(loadReferralSources({ referral_department_id: deptId }));
      ...
      >
        {currentStep === 1 && (
          <Step1
            ...
            onReferralDepartmentChange={handleSelectChange("referralDepartment")}
