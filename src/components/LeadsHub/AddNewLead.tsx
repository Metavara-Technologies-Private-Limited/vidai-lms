...
export default function AddNewLead() {
  ...
  const buildPayload = (): LeadPayload => {
    ...
    const referralDeptName =
      referralDepts.find((d) => d.id === referralDeptId)?.name ?? null;
    ...
    return {
      ...
      // Attach the resolved name so email body can reference it without a second lookup
      _referral_department_name: referralDeptName,
    };
  };
    ...
    try {
      ...
      if (recipientEmail) {
        ...
        const emailBody = [
          ...
          `- Referral Department: ${payload._referral_department_name || "-"}`,
