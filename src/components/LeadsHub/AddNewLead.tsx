...
export default function AddNewLead() {
  ...
  // ── Load pipeline next action type AND status options ─────────────────────
  React.useEffect(() => {
  ...
  // Value-based handlers — used by Step1 dropdowns (no synthetic event needed)
  ...
  const buildPayload = (): LeadPayload => {
    ...
    const backendSafeNextType =
      selectedNextType && isBackendNextActionType(selectedNextType) ? selectedNextType : undefined;
    ...
    // Map active/inactive status to API-compatible values
    const selectedStatus = (form.nextStatus ?? "").trim();
    const resolvedNextActionStatus: "pending" | "completed" | null =
    ...
    return {
      ...
      // next_action_status carries the lifecycle value — no separate lead_status
      next_action_status: resolvedNextActionStatus,
      // lead_status — only send if user explicitly selected a value
      // Backend model must have null=True, blank=True, default=None
      ...
      // lead_generated_by removed — backend uses personal_id + personal_name
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      // Build referral_source object for contracts app (backend object flow)
      ...
      // After booking an appointment, update next_action_status to "completed"
      ...
      >
        {currentStep === 1 && (
          <Step1
            ...
            nextActionStatusOptions={nextActionStatusOptions}
