...
// We use the browser's built-in Intl.DisplayNames API to get the English name
// of every valid BCP-47 language code. This requires zero network calls, zero
// API keys, and returns every language the browser knows about (300+).
// The full list of ISO 639-1 + ISO 639-2 codes we iterate over is sourced from
// the IANA Language Subtag Registry (static — these never change).
const ALL_LANGUAGE_CODES: string[] = [
  ...
  "za","zu",
  // Additional widely-spoken / ISO 639-3 codes browsers usually know
  ...
  try {
    ...
    for (const code of ALL_LANGUAGE_CODES) {
      try {
        ...
        // Skip if the browser just returns the code back (unknown language)
    ...
    // Intl.DisplayNames not supported (very old browsers) — return fallback
    return [
...
// ── Hook: get all languages from browser Intl API (no network call needed) ───
function useUSLanguages() {
  const [languages, setLanguages] = React.useState<string[]>([]);
  const [loading, setLoading] = React.useState(true);

  React.useEffect(() => {
    // Run in a microtask so it doesn't block the initial render
    const id = setTimeout(() => {
...
export function Step1({
  ...
  // ── All languages from browser Intl API (300+ languages, no network call) ──
  ...
  return (
    <Box>
      ...
      {/* Language — all languages via browser Intl.DisplayNames API */}
      <Box sx={{ mb: 4 }}>
