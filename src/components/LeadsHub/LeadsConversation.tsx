      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} mt={1}>
            ...
            {/* Body */}
            <TextField
              fullWidth multiline rows={6} label="Message"
            ...
            <Stack direction="row" spacing={1.5} justifyContent="flex-end">
              ...
              <Button
                ...
                sx={{
                  textTransform: "none", borderRadius: "10px", bgcolor: "#2f2f2f", color: "#fff", px: 2, fontWeight: 600,
                  "&:hover": { bgcolor: "#111" }, "&.Mui-disabled": { bgcolor: "#E2E8F0", color: "#94A3B8" },
                }}
              >
          ...
          <Stack spacing={2} mt={1}>
            <Box sx={{ p: 1.5, bgcolor: "#F8FAFC", borderRadius: "10px", border: "1px solid #E2E8F0" }}>
              ...
              <Typography fontSize={13} fontWeight={600} mt={0.3}>
                {lead.full_name}
                {lead.contact_no && (
                  <Typography component="span" fontSize={12} color="text.secondary" fontWeight={400} ml={1}>
                    {lead.contact_no}
                  </Typography>
                )}
              </Typography>
...
export default function LeadsConversation() {
  ...
  /* set first lead on load */
  React.useEffect(() => {
    if (leads.length > 0 && !activeLead) setActiveLead(leads[0]);
  }, [leads, activeLead]);

  /* fetch history when lead or tab changes */
  React.useEffect(() => {
  ...
  return (
    <Box sx={{ display: "flex", gap: 2, height: "78vh" }}>
      ...
      <Paper
        sx={{
          width: 300, flexShrink: 0, p: 2, borderRadius: "20px",
          border: "1px solid #f1f1f1", display: "flex", flexDirection: "column", overflow: "hidden",
        }}
      >
              ...
              return (
                <Box
                  ...
                  sx={{
                    ...
                    p: 1.5, mb: 1, borderRadius: "14px", cursor: "pointer", background: "#fff",
                    border: isActive ? "1px solid #ff9c6b" : "1px solid transparent",
                    boxShadow: isActive ? "0 4px 12px rgba(255,140,90,0.2)" : "none",
                ...
                >
                  ...
                  <Box sx={{ minWidth: 0, flex: 1 }}>
                    <Stack direction="row" justifyContent="space-between" alignItems="center">
                      <Typography fontSize={13} fontWeight={600} noWrap sx={{ flex: 1, mr: 0.5 }}>
                        {lead.full_name}
                      </Typography>
                      <Chip
                        label={lead.lead_status ?? "New"} size="small"
                        sx={{ bgcolor: style.bg, color: style.color, border: `1px solid ${style.border}`, height: 17, fontSize: 10, flexShrink: 0 }}
                      />
                    </Stack>
        ...
        <Paper sx={{ flex: 1, borderRadius: "20px", border: "1px solid #eee", display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            <Stack direction="row" justifyContent="space-between" alignItems="center" flexWrap="wrap" gap={1}>
              <Stack direction="row" spacing={1.5} alignItems="center">
                ...
                <Box>
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Chip
                      label={activeLead.lead_status ?? "New"} size="small"
                      sx={{ bgcolor: activeStyle.bg, color: activeStyle.color, border: `1px solid ${activeStyle.border}`, height: 20, fontSize: 11 }}
                    />
                  </Stack>
              ...
              <Stack direction="row" spacing={1} flexWrap="wrap">
                {activeLead.department_name && (
                  <Chip label={activeLead.department_name} size="small" sx={{ bgcolor: "#f3f4f6", color: "#374151", fontSize: 11 }} />
                )}
                {activeLead.treatment_interest && (
                  <Chip label={activeLead.treatment_interest} size="small" sx={{ bgcolor: "#ede9fe", color: "#7b61ff", fontSize: 11 }} />
                )}
                {activeLead.source && (
                  <Chip label={`Source: ${activeLead.source}`} size="small" sx={{ bgcolor: "#f0fdf4", color: "#16a34a", fontSize: 11 }} />
                )}
                {activeLead.assigned_to_name && (
                  <Chip label={`Assigned: ${activeLead.assigned_to_name}`} size="small" sx={{ bgcolor: "#fff7ed", color: "#ea580c", fontSize: 11 }} />
                )}
              </Stack>
            </Stack>
          </Box>
              ...
              {TAB_CONFIG.map((t) => (
                <Box
                  ...
                  sx={{
                    ...
                    px: 1.8, py: 0.8, borderRadius: "10px 10px 0 0",
          ...
          <Box sx={{ flex: 1, overflowY: "auto", p: 2.5, bgcolor: "#F8FAFC" }}>
              ...
              <>
                ...
                {!emailLoading && emailError && (
                  <Alert severity="error" sx={{ borderRadius: "10px" }} action={
                    <Button size="small" onClick={() => fetchEmails(activeLead.id)}>Retry</Button>
                  }>{emailError}</Alert>
                )}
                  ...
                  <Stack spacing={2}>
                    {emailHistory.map((mail) => (
                      <Paper key={mail.id} elevation={0} sx={{ p: 2.5, borderRadius: "14px", border: "1px solid #E2E8F0", bgcolor: "#fff" }}>
                        ...
                        <Divider sx={{ mb: 1.5 }} />
                        <Box
                          sx={{ fontSize: 13, color: "text.secondary", lineHeight: 1.75, "& p": { margin: "0 0 8px 0" }, "& a": { color: "#3B82F6" }, "& strong": { fontWeight: 700 } }}
            ...
            {activeTab === "sms" && (
              <>
                ...
                {!smsLoading && smsError && (
                  <Alert severity="error" sx={{ borderRadius: "10px" }} action={
                    <Button size="small" onClick={() => fetchSMS(activeLead.id)}>Retry</Button>
                  }>{smsError}</Alert>
                )}
                          ...
                          <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1.5}>
                            ...
                            <Stack alignItems="flex-end" spacing={0.5}>
                              <Chip label={sms.status || "sent"} size="small"
                                sx={{ bgcolor: statusStyle.bg, color: statusStyle.color, fontWeight: 600, fontSize: 11, height: 20, textTransform: "capitalize" }}
                              />
                              <Typography variant="caption" color="text.secondary" fontSize={11}>
            ...
            {activeTab === "call" && (
              <>
                ...
                {!callLoading && callError && (
                  <Alert severity="error" sx={{ borderRadius: "10px" }} action={
                    <Button size="small" onClick={() => fetchCalls(activeLead.id)}>Retry</Button>
                  }>{callError}</Alert>
                )}
          ...
          <Box sx={{ p: 2, borderTop: "1px solid #f1f2f6", flexShrink: 0 }}>
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Box
                sx={{ flex: 1, display: "flex", alignItems: "center", bgcolor: "#f6f7fb", borderRadius: "14px", px: 2, py: 1, cursor: "pointer", gap: 1 }}
                onClick={handleCompose}
              >
                {activeTab === "sms"
              ...
              <IconButton
                onClick={handleCompose}
                sx={{ bgcolor: "#2f2f2f", color: "#fff", borderRadius: "12px", width: 40, height: 40, "&:hover": { bgcolor: "#111" } }}
              >
