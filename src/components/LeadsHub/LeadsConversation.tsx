...
const USE_CASE_OPTIONS = ["appointment", "follow-up", "reminder", "promotion", "general"];
...
const saveNewSMSTemplate = async (payload: { name: string; body: string; use_case?: string }): Promise<SMSTemplate> => {
  const res = await fetch("/api/sms-templates/", {
    method: "POST",
    headers: {
...
const addEmailsFromInput = (
  ...
  const emails = raw
    .split(/[\s,;]+/)
    .map((e) => e.trim())
    .filter((e) => isValidEmail(e) && !existing.map(normalizeEmail).includes(normalizeEmail(e)));
...
const SMSSuite: React.FC<SMSSuiteProps> = ({ open, lead, onClose, onSent }) => {
  ...
  React.useEffect(() => {
    if (!open) return;
    setStep("compose"); setSmsMsg(""); setSmsError(null);
    setSelectedTpl(null); setPreviewBody("");
    setNewTplView("form"); setNewTplName(""); setNewTplBody("");
    setNewTplUseCase(""); setNewTplError(null);
  }, [open]);
  ...
  const openTemplates = () => { loadTemplates(); setStep("templates"); };
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      await TwilioAPI.sendSMS({ lead_uuid: lead.id, to: lead.contact_no, message: smsMessage.trim() });
                      ...
                      <ListItemText
                        primary={
                          <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            {tpl.use_case && (
                              <Chip label={tpl.use_case} size="small" sx={{ ...getUseCaseSx(tpl.use_case), fontSize: "11px", height: 20, textTransform: "capitalize" }} />
                            )}
                          </Stack>
                        }
                        secondary={
      ...
      <Dialog open={open && step === "preview"} onClose={() => setStep("templates")} maxWidth="sm" fullWidth PaperProps={{ sx: dialogSx }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2}>
            {selectedTpl && (
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                {selectedTpl.use_case && (
                  <Chip label={selectedTpl.use_case} size="small" sx={{ ...getUseCaseSx(selectedTpl.use_case), fontSize: "11px", height: 20, textTransform: "capitalize" }} />
                )}
              </Stack>
            )}
            {/* SMS bubble preview */}
            ...
            <TextField
              label="Edit message before sending" multiline rows={4}
              value={previewBody} onChange={(e) => setPreviewBody(e.target.value)}
        ...
        <DialogActions sx={{ px: 3, pb: 3, gap: 1 }}>
          <Button fullWidth onClick={() => setStep("templates")}
            sx={{ height: 44, bgcolor: "#F3F4F6", color: "black", fontWeight: 500, textTransform: "none", borderRadius: "8px", "&:hover": { bgcolor: "#E5E7EB" } }}>
            Back
          </Button>
          ...
          <>
                ...
                <Box>
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      border: "1px solid", borderColor: useCaseAnchor ? "#1976d2" : "#D1D5DB",
                      borderRadius: "8px", px: 1.5, cursor: "pointer", minHeight: 42, bgcolor: "#fff",
                      boxShadow: useCaseAnchor ? "0 0 0 2px rgba(25,118,210,0.15)" : "none",
                      "&:hover": { borderColor: "#9CA3AF" }, transition: "all 0.15s",
                    }}>
                    {newTplUseCase
                  ...
                  <Menu
                    ...
                    {USE_CASE_OPTIONS.map((uc) => (
                      <MenuItem
                        ...
                        onClick={() => { setNewTplUseCase(uc); setUseCaseAnchor(null); }}
            ...
            <DialogActions sx={{ px: 3, pb: 3, pt: 1, gap: 1 }}>
              ...
              <Button
                ...
                disabled={newTplSaving || !newTplName.trim() || !newTplBody.trim()}
...
const EmailSuite: React.FC<EmailSuiteProps> = ({ open, lead, onClose, onSent }) => {
  ...
  const recipients = React.useMemo(() =>
    lead.email ? [{ id: lead.id, name: lead.full_name, email: lead.email }] : [],
  [lead.email, lead.id, lead.full_name]);

  const filteredToRecipients  = recipients.filter((r) => !replyTo.map(normalizeEmail).includes(normalizeEmail(r.email)));
  const filteredCcRecipients  = recipients.filter((r) => !replyCc.map(normalizeEmail).includes(normalizeEmail(r.email)));
  const filteredBccRecipients = recipients.filter((r) => !replyBcc.map(normalizeEmail).includes(normalizeEmail(r.email)));
  ...
  const toggleRecipient = (email: string, _list: string[], setter: React.Dispatch<React.SetStateAction<string[]>>) => {
    setter((prev) =>
      prev.map(normalizeEmail).includes(normalizeEmail(email))
        ? prev.filter((e) => normalizeEmail(e) !== normalizeEmail(email))
        : [...prev, email]
  ...
  const handleSendEmail = async () => {
    ...
    try {
      await LeadEmailAPI.sendNow({
        ...
        ...(replyTo.filter((e) => e !== lead.email).length > 0 && { additional_to: replyTo.filter((e) => e !== lead.email) }),
  ...
  return (
    <>
      {/* SELECTOR DIALOG */}
      <Dialog open={open && step === "selector"} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: dialogSx }}>
        <Box sx={{ px: 3, py: 2, borderBottom: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Box>
            <Typography fontWeight={600}>New Email</Typography>
            <Typography variant="caption" color="text.secondary">Sending to {lead.full_name}</Typography>
          </Box>
        ...
        <DialogContent>
          ...
          <Box
            ...
            onClick={() => { setSelectedTemplate(null); setSubject(""); setMessageBody(""); goToCompose(); }}>
          ...
          {!templatesLoading && templatesError && (
            <Alert severity="error" sx={{ borderRadius: "10px", mb: 2 }} action={<Button size="small" onClick={loadTemplates}>Retry</Button>}>{templatesError}</Alert>
          )}
          ...
          {!templatesLoading && !templatesError && templates.map((t) => (
            <Box key={t.id}
              ...
              {(t as EmailTemplate & { use_case?: string }).use_case && (
                <Chip label={(t as EmailTemplate & { use_case?: string }).use_case} size="small"
                  sx={{ ...getUseCaseSx((t as EmailTemplate & { use_case?: string }).use_case!), fontSize: "11px", height: 20, mr: 1, textTransform: "capitalize" }} />
              )}
        ...
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={onClose} variant="outlined" sx={{ borderColor: "#D1D5DB", color: "#374151" }}>Cancel</Button>
          <Button onClick={goToCompose} variant="contained" disabled={!selectedTemplate}
            sx={{ bgcolor: "#111827", "&:hover": { bgcolor: "#000" }, "&:disabled": { bgcolor: "#E5E7EB", color: "#9CA3AF" } }}>
            Continue
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={!!previewTemplate} onClose={() => setPreviewTemplate(null)} maxWidth="sm" fullWidth PaperProps={{ sx: dialogSx }}>
        <Box sx={{ px: 3, py: 2, borderBottom: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Box>
            <Typography fontWeight={600}>{previewTemplate?.name}</Typography>
            {(previewTemplate as (EmailTemplate & { use_case?: string }) | null)?.use_case && (
              <Chip label={(previewTemplate as EmailTemplate & { use_case?: string }).use_case} size="small"
                sx={{ ...getUseCaseSx((previewTemplate as EmailTemplate & { use_case?: string }).use_case!), fontSize: "11px", height: 20, mt: 0.5, textTransform: "capitalize" }} />
            )}
          </Box>
      ...
      {/* COMPOSE DIALOG */}
      <Dialog open={open && step === "compose"} onClose={() => !isSending && onClose()} maxWidth="md" fullWidth PaperProps={{ sx: dialogSx }}>
        <Box sx={{ px: 3, py: 2, borderBottom: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <Box>
            <Typography fontWeight={600}>{selectedTemplate ? selectedTemplate.name : "New Email"}</Typography>
            <Typography variant="caption" color="text.secondary">Sending to {replyTo.length} recipient{replyTo.length > 1 ? "s" : ""}</Typography>
          </Box>
          <IconButton onClick={onClose} disabled={isSending}><CloseIcon /></IconButton>
        </Box>
        <DialogContent sx={{ px: 3, pb: 1 }}>
          ...
          <Box display="flex" alignItems="center" gap={1} pb={1.2} borderBottom="1px solid #E6E6E6">
            ...
            <TextField select value={fromEmail} onChange={(e) => setFromEmail(e.target.value)} variant="standard"
              sx={{ minWidth: 260, "& .MuiInputBase-input": { fontSize: 14 } }} InputProps={{ disableUnderline: true }}>
              {fromOptions.map((email) => <MenuItem key={email} value={email} sx={{ fontSize: 14 }}>{email}</MenuItem>)}
            </TextField>
          </Box>

          {/* TO */}
          <Box ref={toRowRef} display="flex" alignItems="flex-start" gap={1} py={1.5} borderBottom="1px solid #E6E6E6" sx={{ flexWrap: "wrap" }}>
            <Typography fontSize={14} color="#7A7A7A" minWidth={48}>To :</Typography>
            <Box display="flex" gap={1} flexWrap="wrap" flex={1} minWidth={180}
              sx={{ maxHeight: "90px", overflowY: "auto", paddingRight: "8px", "&::-webkit-scrollbar": { width: "6px" }, "&::-webkit-scrollbar-track": { bgcolor: "#F3F4F6", borderRadius: "3px" }, "&::-webkit-scrollbar-thumb": { bgcolor: "#D1D5DB", borderRadius: "3px" } }}>
              {replyTo.map((email) => (
                <Box key={email} display="flex" alignItems="center" gap={1} px={1.2} py={0.5} borderRadius="16px" bgcolor="#F1F3F5">
                  ...
                  <Box component="span" onClick={() => setReplyTo(replyTo.filter((m) => m !== email))} sx={{ cursor: "pointer", fontSize: 16, color: "#7A7A7A", "&:hover": { color: "#000" } }}>×</Box>
                </Box>
              ))}
              <InputBase value={toInput} placeholder="Add TO recipients"
                onFocus={() => { if (recipients.length > 0 && toRowRef.current) setToAnchorEl(toRowRef.current); }}
                onChange={(e) => setToInput(e.target.value)}
                onBlur={() => { addEmailsFromInput(toInput, replyTo, setReplyTo); setToInput(""); }}
                onKeyDown={(e) => { if (["Enter", ",", "Tab"].includes(e.key)) { e.preventDefault(); addEmailsFromInput(toInput, replyTo, setReplyTo); setToInput(""); } }}
                sx={{ minWidth: 140, fontSize: 14, flex: 1 }} />
            </Box>
            <Box display="flex" gap={1} ml="auto" pt={0.5}>
              <Typography onClick={() => setShowCc(!showCc)} sx={{ fontSize: 13, cursor: "pointer", color: showCc || replyCc.length > 0 ? "#232323" : "#9E9E9E", fontWeight: showCc || replyCc.length > 0 ? 600 : 400 }}>Cc</Typography>
              <Typography sx={{ fontSize: 13, color: "#9E9E9E", fontWeight: 600 }}>|</Typography>
              <Typography onClick={() => setShowBcc(!showBcc)} sx={{ fontSize: 13, cursor: "pointer", color: showBcc || replyBcc.length > 0 ? "#232323" : "#9E9E9E", fontWeight: showBcc || replyBcc.length > 0 ? 600 : 400 }}>Bcc</Typography>
            </Box>
            <Popover open={!!toAnchorEl} anchorEl={toAnchorEl} onClose={() => setToAnchorEl(null)}
              anchorOrigin={{ vertical: "bottom", horizontal: "left" }} transformOrigin={{ vertical: "top", horizontal: "left" }}
              disableAutoFocus disableEnforceFocus disableRestoreFocus
              PaperProps={{ ref: (node: HTMLDivElement) => { toPickerPaperRef.current = node; }, onMouseDown: (e: React.MouseEvent<HTMLDivElement>) => e.preventDefault() }}>
              <Box sx={{ width: 320, maxHeight: 260, overflowY: "auto", p: 1 }}>
                {filteredToRecipients.length === 0
                  ...
                  : filteredToRecipients.map((r) => (
                    <Box key={r.id} onClick={() => toggleToRecipient(r.email)} sx={{ p: 0.8, borderRadius: 1, cursor: "pointer", display: "flex", alignItems: "center", gap: 1, "&:hover": { bgcolor: "#F5F5F5" } }}>
                      <Checkbox size="small" checked={replyTo.map(normalizeEmail).includes(normalizeEmail(r.email))} onChange={() => toggleToRecipient(r.email)} />
                      <Box><Typography fontSize={13} fontWeight={500}>{r.name}</Typography><Typography fontSize={12} color="text.secondary">{r.email}</Typography></Box>
                    </Box>
                  ))
                }
              </Box>
          ...
          {(showCc || replyCc.length > 0) && (
            <Box ref={ccFieldRef} display="flex" alignItems="center" gap={1} py={1} borderBottom="1px solid #E6E6E6">
              <Typography fontSize={14} color="#7A7A7A" minWidth={35}>Cc :</Typography>
              <Box display="flex" gap={1} flexWrap="wrap" flex={1}>
                {replyCc.map((mail) => (
                  <Box key={mail} display="flex" alignItems="center" gap={1} px={1.2} py={0.4} borderRadius="16px" bgcolor="#F1F3F5">
                    <Typography fontSize={13}>{mail}</Typography>
                    <Box component="span" sx={{ cursor: "pointer", fontSize: 15, color: "#666" }} onClick={() => setReplyCc(replyCc.filter((item) => normalizeEmail(item) !== normalizeEmail(mail)))}>×</Box>
                  </Box>
                ))}
                <InputBase value={ccInput} placeholder="Add CC recipients"
                  onFocus={() => setCcAnchorEl(ccFieldRef.current)}
                  onChange={(e) => setCcInput(e.target.value)}
                  onBlur={() => { addEmailsFromInput(ccInput, replyCc, setReplyCc); setCcInput(""); }}
                  onKeyDown={(e) => { if (["Enter", ",", "Tab"].includes(e.key)) { e.preventDefault(); addEmailsFromInput(ccInput, replyCc, setReplyCc); setCcInput(""); } }}
                  sx={{ minWidth: 180, fontSize: 14, flex: 1 }} />
              </Box>
              <Popover open={!!ccAnchorEl} anchorEl={ccAnchorEl} onClose={() => setCcAnchorEl(null)}
                anchorOrigin={{ vertical: "bottom", horizontal: "left" }} transformOrigin={{ vertical: "top", horizontal: "left" }}
                disableAutoFocus disableEnforceFocus disableRestoreFocus
                PaperProps={{ ref: (node: HTMLDivElement) => { ccPickerPaperRef.current = node; }, onMouseDown: (e: React.MouseEvent<HTMLDivElement>) => e.preventDefault() }}>
                <Box sx={{ width: 320, maxHeight: 240, overflowY: "auto", p: 1 }}>
                  {filteredCcRecipients.length === 0
                    ? <Typography fontSize={12} color="text.secondary" p={1}>No matching recipients</Typography>
                    : filteredCcRecipients.map((r) => (
                      <Box key={`cc-${r.id}`} onClick={() => toggleRecipient(r.email, replyCc, setReplyCc)} sx={{ p: 0.8, borderRadius: 1, cursor: "pointer", display: "flex", alignItems: "center", gap: 1, "&:hover": { bgcolor: "#F5F5F5" } }}>
                        <Checkbox size="small" checked={replyCc.map(normalizeEmail).includes(normalizeEmail(r.email))} onChange={() => toggleRecipient(r.email, replyCc, setReplyCc)} />
                        <Box><Typography fontSize={13} fontWeight={500}>{r.name}</Typography><Typography fontSize={12} color="text.secondary">{r.email}</Typography></Box>
                      </Box>
                    ))
                  }
          ...
          {(showBcc || replyBcc.length > 0) && (
            <Box ref={bccFieldRef} display="flex" alignItems="center" gap={1} py={1} borderBottom="1px solid #E6E6E6">
              <Typography fontSize={14} color="#7A7A7A" minWidth={40}>Bcc :</Typography>
              <Box display="flex" gap={1} flexWrap="wrap" flex={1}>
                {replyBcc.map((mail) => (
                  <Box key={mail} display="flex" alignItems="center" gap={1} px={1.2} py={0.4} borderRadius="16px" bgcolor="#F1F3F5">
                    <Typography fontSize={13}>{mail}</Typography>
                    <Box component="span" sx={{ cursor: "pointer", fontSize: 15, color: "#666" }} onClick={() => setReplyBcc(replyBcc.filter((item) => normalizeEmail(item) !== normalizeEmail(mail)))}>×</Box>
                  </Box>
                ))}
                <InputBase value={bccInput} placeholder="Add BCC recipients"
                  onFocus={() => setBccAnchorEl(bccFieldRef.current)}
                  onChange={(e) => setBccInput(e.target.value)}
                  onBlur={() => { addEmailsFromInput(bccInput, replyBcc, setReplyBcc); setBccInput(""); }}
                  onKeyDown={(e) => { if (["Enter", ",", "Tab"].includes(e.key)) { e.preventDefault(); addEmailsFromInput(bccInput, replyBcc, setReplyBcc); setBccInput(""); } }}
                  sx={{ minWidth: 180, fontSize: 14, flex: 1 }} />
              </Box>
              <Popover open={!!bccAnchorEl} anchorEl={bccAnchorEl} onClose={() => setBccAnchorEl(null)}
                anchorOrigin={{ vertical: "bottom", horizontal: "left" }} transformOrigin={{ vertical: "top", horizontal: "left" }}
                disableAutoFocus disableEnforceFocus disableRestoreFocus
                PaperProps={{ ref: (node: HTMLDivElement) => { bccPickerPaperRef.current = node; }, onMouseDown: (e: React.MouseEvent<HTMLDivElement>) => e.preventDefault() }}>
                <Box sx={{ width: 320, maxHeight: 240, overflowY: "auto", p: 1 }}>
                  {filteredBccRecipients.length === 0
                    ? <Typography fontSize={12} color="text.secondary" p={1}>No matching recipients</Typography>
                    : filteredBccRecipients.map((r) => (
                      <Box key={`bcc-${r.id}`} onClick={() => toggleRecipient(r.email, replyBcc, setReplyBcc)} sx={{ p: 0.8, borderRadius: 1, cursor: "pointer", display: "flex", alignItems: "center", gap: 1, "&:hover": { bgcolor: "#F5F5F5" } }}>
                        <Checkbox size="small" checked={replyBcc.map(normalizeEmail).includes(normalizeEmail(r.email))} onChange={() => toggleRecipient(r.email, replyBcc, setReplyBcc)} />
                        <Box><Typography fontSize={13} fontWeight={500}>{r.name}</Typography><Typography fontSize={12} color="text.secondary">{r.email}</Typography></Box>
                      </Box>
                    ))
                  }
        ...
        <Box sx={{ px: 3, py: 2, borderTop: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Stack direction="row" spacing={1}>
            <Button onClick={() => setStep("selector")} variant="outlined" disabled={isSending} sx={{ borderColor: "#D1D5DB", color: "#374151" }}>Back</Button>
            <Button variant="contained" disabled sx={{ bgcolor: "#F3F4F6", color: "#9CA3AF" }}>Save as Template</Button>
            <Button variant="contained"
              endIcon={isSending ? <CircularProgress size={16} sx={{ color: "white" }} /> : <SendOutlinedIcon />}
              onClick={handleSendEmail}
              disabled={!subject.trim() || !messageBody.trim() || replyTo.length === 0 || isSending}
              sx={{ bgcolor: "#4B5563", "&:hover": { bgcolor: "#374151" }, "&:disabled": { bgcolor: "#E5E7EB", color: "#9CA3AF" }, minWidth: 90 }}>
...
export default function LeadsConversation() {
  ...
  const fetchEmails = async (id: string) => {
    ...
    try { const data = await LeadEmailAPI.listByLead(id); setEmailHistory(Array.isArray(data) ? data : []); }
