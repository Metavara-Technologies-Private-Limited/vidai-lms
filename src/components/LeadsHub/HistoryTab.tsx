...
const getBotReply = (userText: string): string => {
  ...
  if (lower.includes("appointment"))
    return "I can help you with appointments. Please share your preferred date and time.";
  if (lower.includes("hello") || lower.includes("hi"))
    return "Hello! How can I assist you today?";
  if (lower.includes("treatment"))
    return "We offer a wide range of treatments. Could you let us know what you're interested in?";
  if (
    lower.includes("cost") ||
    lower.includes("price") ||
    lower.includes("fee")
  )
    return "Our team will get in touch with you shortly regarding pricing details.";
  if (lower.includes("doctor") || lower.includes("consultant"))
    return "Our experienced consultants are available Monday–Saturday. Would you like to book a slot?";
  if (
    lower.includes("contact") ||
    lower.includes("phone") ||
    lower.includes("call")
  )
    return "You can reach us at our clinic number. Alternatively, we can arrange a callback for you.";
...
const RecipientField: React.FC<RecipientFieldProps> = ({
  ...
  const commit = (val: string) => {
    ...
    if (trimmed && isValidEmail(trimmed) && !chips.includes(trimmed))
      onAdd(trimmed);
    setInputValue("");
    setSuggestions([]);
    setOpen(false);
    ...
    >
            ...
            {suggestions.map((s, idx) => (
              ...
              >
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Typography fontSize="11.5px" color="#64748B" noWrap>
                    {s.email}
                  </Typography>
                </Box>
                {idx === highlightIdx && (
                  <Typography fontSize="10px" color="#93C5FD" fontWeight={500}>
                    ↵
                  </Typography>
                )}
              </Box>
      ...
      {onDismiss && (
        <IconButton
          size="small"
          onClick={onDismiss}
          sx={{ color: "#CBD5E1", mt: "2px", flexShrink: 0 }}
        >
...
const ComposeEmailDialog: React.FC<ComposeEmailDialogProps> = ({
  ...
  React.useEffect(() => {
    ...
    EmailTemplateAPI.list()
      ...
      .catch((err) =>
        setTemplatesError(
          extractErrorMessage(err, "Failed to load email templates."),
        ),
      )
  ...
  const addEmailsFromInput = (
    ...
    chunks.forEach((mail) => {
      if (
        isValidEmail(mail) &&
        !next.some((m) => normalizeEmailAddr(m) === normalizeEmailAddr(mail))
      )
        next.push(mail);
    });
    setList(next);
  ...
  const toggleToEmail = (email: string) => {
    const exists = toEmails.some(
      (m) => normalizeEmailAddr(m) === normalizeEmailAddr(email),
    );
    if (exists)
      setToEmails(
        toEmails.filter(
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await LeadEmailAPI.sendNow(
        payload as Parameters<typeof LeadEmailAPI.sendNow>[0],
      );
            ...
            <Box display="flex" gap={1} ml="auto" pt={0.5}>
              <Typography
                ...
                sx={{
                  fontSize: 13,
                  cursor: "pointer",
                  color: showCc || ccEmails.length > 0 ? "#232323" : "#9E9E9E",
                  fontWeight: showCc || ccEmails.length > 0 ? 600 : 400,
                }}
              >
                Cc
              </Typography>
              ...
              <Typography
                ...
                sx={{
                  ...
                  cursor: "pointer",
                  color:
                    showBcc || bccEmails.length > 0 ? "#232323" : "#9E9E9E",
                  fontWeight: showBcc || bccEmails.length > 0 ? 600 : 400,
                }}
              >
                Bcc
              </Typography>
                ...
                {leadRecipient.length === 0 ? (
                  ...
                  leadRecipient.map((r) => (
                    ...
                    >
                      <Checkbox
                        size="small"
                        checked={toEmails.some(
                          (m) =>
                            normalizeEmailAddr(m) ===
                            normalizeEmailAddr(r.email),
                        )}
                  ...
                  <EmailChipRow
                    ...
                    onRemove={() =>
                      setCcEmails(
                        ccEmails.filter(
                          (m) =>
                            normalizeEmailAddr(m) !== normalizeEmailAddr(mail),
                        ),
                      )
                    }
                  />
              ...
              >
                {hasAppointment ? (
                  ...
                  >
                    ...
                    >
                      <Box
                        sx={{ p: 1, bgcolor: "#ECFDF5", borderRadius: "8px" }}
                      >
                        <EventNoteIcon
                    ...
                    <Stack spacing={2.5}>
                      <Stack direction="row" spacing={4}>
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Typography fontWeight={700} fontSize="14px" mt={0.3}>
                            {appointmentDate}
                          </Typography>
                        </Box>
                        <Box sx={{ flex: 1 }}>
                      ...
                      <Stack direction="row" spacing={4}>
                        <Box sx={{ flex: 1 }}>
                          ...
                          <Typography fontWeight={600} fontSize="14px" mt={0.3}>
                            {appointmentDepartment}
                          </Typography>
                        </Box>
                        <Box sx={{ flex: 1 }}>
                              ...
                              sx={{
                                ...
                                borderRadius:
                                  msg.role === "user"
                                    ...
                                    : "18px 18px 18px 4px",
                                bgcolor:
                                  msg.role === "user" ? "#EDE9FE" : "#F8FAFC",
                                border: "1px solid",
                                borderColor:
                                  msg.role === "user" ? "#DDD6FE" : "#E2E8F0",
                              }}
                            >
                              <Typography
                                fontSize="13.5px"
                                sx={{ color: "#1E293B", lineHeight: 1.65 }}
                              >
                                {msg.text}
                              </Typography>
                            </Box>
                ...
                <Stack direction="row" spacing={1} alignItems="flex-end">
                  ...
                  <IconButton
                    ...
                    sx={{
                      ...
                      flexShrink: 0,
                      bgcolor:
                        chatInput.trim() && !botTyping ? "#1E293B" : "#E2E8F0",
                      color:
                        chatInput.trim() && !botTyping ? "#FFFFFF" : "#94A3B8",
                      transition: "all 0.15s",
                      "&:hover": {
                        bgcolor:
                          chatInput.trim() && !botTyping
                            ? "#0F172A"
                            : "#E2E8F0",
                      },
                      "&.Mui-disabled": {
                    ...
                    {emailHistory.map((mail) => (
                      ...
                      >
                        ...
                        <Stack spacing={0.4} mb={1}>
                          ...
                          {(
                            (mail as LeadMailListItem & { cc?: string[] }).cc ??
                            []
                          ).length > 0 && (
                            <Stack
                              direction="row"
                            ...
                            >
                              ...
                              >
                                {(
                                  (mail as LeadMailListItem & { cc?: string[] })
                                    .cc ?? []
                                ).map((e: string) => (
                                  <Chip
                                    key={e}
                          ...
                          {(
                            (mail as LeadMailListItem & { bcc?: string[] })
                              .bcc ?? []
                          ).length > 0 && (
                            <Stack
                              direction="row"
