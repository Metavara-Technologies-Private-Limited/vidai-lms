...
import { Modal, Box, IconButton, Typography, FormControl, Select, MenuItem } from "@mui/material";
...
<<<<<<< Updated upstream:src/components/Campaign/DuplicateCampaignModal.tsx
...
interface DuplicateCampaignModalProps {
>>>>>>> Stashed changes:src/components/Layout/Campaign/DuplicateCampaignModal.tsx
  campaign: any;
  onClose: () => void;
  onSave: (updatedCampaign: any) => void;
}

export default function DuplicateCampaignModal({ campaign, onClose, onSave }: DuplicateCampaignModalProps) {
  const [step, setStep] = useState(1);
  const [submitted, setSubmitted] = useState(false);
  const [campaignName, setCampaignName] = useState(campaign.name + " (Copy)");
  const [campaignDescription, setCampaignDescription] = useState(campaign.campaign_description || "");
  ...
  const [accounts, setAccounts] = useState<string[]>(campaign.platforms || []);
  const [mode, setMode] = useState<"organic" | "paid" | "">("");
  const [scheduleDate, setScheduleDate] = useState(campaign.scheduledAt ? dayjs(campaign.scheduledAt).format("YYYY-MM-DD") : "");
  const [scheduleTime, setScheduleTime] = useState(campaign.scheduledAt ? dayjs(campaign.scheduledAt).format("HH:mm") : "");
    ...
    const fetchCampaign = async () => {
      try {
        ...
        if (data.social_media?.length > 0) {
          setAccounts(data.social_media.map((sm: any) => sm.platform_name));
        }
...
=======
  const step1Valid = campaignName.trim() && campaignDescription.trim() && objective && audience && startDate && endDate;
  const step2Valid = campaign.type === "email" ? audience && subject.trim() && emailBody.trim() : accounts.length > 0 && mode;
>>>>>>> Stashed changes:src/components/Layout/Campaign/DuplicateCampaignModal.tsx
  ...
  const handleSubmit = async () => {
    ...
    try {
      const scheduledDateTime = dayjs(`${scheduleDate} ${scheduleTime}`, "YYYY-MM-DD HH:mm").format("YYYY-MM-DDTHH:mm:ss");
      const payload = {
        clinic: 1,
        ...
        email: campaign.type === "email" ? [{ audience_name: audience, subject, email_body: emailBody, template_name: "EMAIL", sender_email: "noreply@clinic.com", scheduled_at: scheduledDateTime, is_active: true }] : [],
        social_media: campaign.type === "social" ? accounts.map((platform) => ({ platform_name: platform, is_active: true })) : [],
      };
  ...
  const audienceLabel = audience === "all" ? "All Subscribers" : audience === "active" ? "Active Users" : "";
...
=======
      <Modal open={true} onClose={onClose}>
        <Box className="email-campaign-modal">
          <div className="add-modal-header">
            <Typography variant="h6">Duplicate {campaign.type === "email" ? "Email" : "Social Media"} Campaign</Typography>
...
>>>>>>> Stashed changes:src/components/Layout/Campaign/DuplicateCampaignModal.tsx
          ...
          <div className="stepper">
            <div className={`step ${step === 1 ? "active" : ""} ${step > 1 ? "completed" : ""}`}>
              <div className="circle">{step > 1 ? "✓" : "1"}</div><span>Campaign Details</span>
            </div>
            <div className="line" />
          ...
          {step === 1 && (
            <div className="step-content">
              ...
              <div className={`form-group ${submitted && !campaignDescription ? "error" : ""}`}>
                ...
                <input value={campaignDescription} onChange={(e) => setCampaignDescription(e.target.value)} placeholder="Short description of campaign" />
              </div>
            ...
            <div className="step-content">
              ...
              <div className={`section-card ${submitted && (!subject || !emailBody) ? "error" : ""}`}>
                <div className="email-content-header">
                  <div><h3>Email Content</h3><p className="section-subtitle">Design your email with AI assistance</p></div>
                  <div className="email-actions">
                    <button className="outline-btn" onClick={() => setPreviewOpen(!previewOpen)}>
                      <img src={viewIcon} alt="View" width={20} height={20} />Preview Email
                    </button>
                    <button className="light-btn" onClick={() => setTemplateOpen(true)}>+ Email Template</button>
                  </div>
                </div>
                <div className="email-body-row">
                  <div className="email-left">
                    ...
                    <div className={`form-group ${submitted && !emailBody ? "error" : ""}`}>
                      <label>Email *</label>
                      <textarea value={emailBody} onChange={(e) => setEmailBody(e.target.value)} placeholder="Enter email content" rows={8} />
                      <span className="ai-suggest">✨ AI Suggest</span>
                    </div>
                  </div>
                  {previewOpen && (
                    <div className="email-preview">
                      <div className="preview-header"><h3>Preview Email</h3><button onClick={() => setPreviewOpen(false)}>✕</button></div>
                      <div className="preview-body">
                        ...
                        <div className="preview-divider"></div>
                        <div className="preview-email-content"><p>{emailBody}</p></div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="step-content">
              ...
              <div className={`section-card ${submitted && !mode ? "error" : ""}`}>
                ...
                <div className="mode-row">
                  <div className={`mode-card ${mode === "organic" ? "selected" : ""}`} onClick={() => setMode("organic")}>
                    <div className="mode-left"><div className={`radio ${mode === "organic" ? "checked" : ""}`} /><div className="mode-text"><h4>Organic Posting</h4><p>Post without ad spend.</p></div></div>
                    <span className="badge">No Budget Required</span>
                  </div>
                  <div className={`mode-card ${mode === "paid" ? "selected" : ""}`} onClick={() => setMode("paid")}>
                    <div className="mode-left"><div className={`radio ${mode === "paid" ? "checked" : ""}`} /><div className="mode-text"><h4>Paid Advertising</h4><p>Boost reach with targeted ads.</p></div></div>
              ...
              {mode && (
                <div className="section-card">
                  ...
                  <div className="content-row"><img src={instagramIcon} alt="Instagram" /><textarea placeholder="Instagram content" disabled={!accounts.includes("instagram")} style={{ opacity: accounts.includes("instagram") ? 1 : 0.5 }} /></div>
                  <div className="content-row"><img src={facebookIcon} alt="Facebook" /><input placeholder="Facebook content" disabled={!accounts.includes("facebook")} style={{ opacity: accounts.includes("facebook") ? 1 : 0.5 }} /></div>
                  <div className="content-row"><img src={linkedinIcon} alt="LinkedIn" /><input placeholder="LinkedIn content" disabled={!accounts.includes("linkedin")} style={{ opacity: accounts.includes("linkedin") ? 1 : 0.5 }} /></div>
                </div>
              )}
            </div>
          )}
            ...
            <div className="step-content">
              ...
              <div className="schedule-card">
                <div className="schedule-header">
                  <div className="schedule-title"><h3>Schedule</h3><p>Select date and time to send the email</p></div>
                ...
                <div className="schedule-row">
                  ...
                  <div className={`schedule-field ${submitted && !scheduleTime ? "error" : ""}`}>
                    ...
                    <input className="schedule-input" type="time" value={scheduleTime} onChange={(e) => setScheduleTime(e.target.value)} />
                  </div>
                </div>
              </div>
            </div>
          ...
          {step === 3 && campaign.type === "social" && (
            <div className="step-content">
              ...
              <div className="section-card">
                <div className="schedule-header">
                  <div><h3>{mode === "paid" ? "Schedule & Budget Allocation" : "Schedule"}</h3></div>
                  ...
                  <>
                    ...
                    <div className="budget-section">
                      ...
                      <div className="budget-row">
                        {accounts.includes("instagram") && <div className="budget-card"><div className="budget-title"><img src={instagramIcon} alt="Instagram" /><span>Instagram ($3.5 CPC)</span></div><div className="budget-input-wrapper"><label>Amount ($)</label><input type="number" min="0" step="10" value={instagramBudget} onChange={(e) => setInstagramBudget(Number(e.target.value))} className="budget-input" /></div></div>}
                        {accounts.includes("facebook") && <div className="budget-card"><div className="budget-title"><img src={facebookIcon} alt="Facebook" /><span>Facebook ($2.5 CPC)</span></div><div className="budget-input-wrapper"><label>Amount ($)</label><input type="number" min="0" step="10" value={facebookBudget} onChange={(e) => setFacebookBudget(Number(e.target.value))} className="budget-input" /></div></div>}
                        {accounts.includes("linkedin") && <div className="budget-card"><div className="budget-title"><img src={linkedinIcon} alt="LinkedIn" /><span>LinkedIn ($1.5 CPC)</span></div><div className="budget-input-wrapper"><label>Amount ($)</label><input type="number" min="0" step="10" value={linkedinBudget} onChange={(e) => setLinkedinBudget(Number(e.target.value))} className="budget-input" /></div></div>}
                      </div>
                      <div className="total-budget"><div><h4>Total: ${(accounts.includes("instagram") ? instagramBudget : 0) + (accounts.includes("facebook") ? facebookBudget : 0) + (accounts.includes("linkedin") ? linkedinBudget : 0)}</h4></div></div>
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="modal-actions">
            ...
            {step === 3 ? (
              <button className="next-btn" onClick={handleSubmit}>Submit Campaign</button>
            ) : (
      ...
      <EmailTemplateModal open={templateOpen} onClose={() => setTemplateOpen(false)}
        onSelect={(template: any) => { setSubject(template.title); setEmailBody(template.subtitle); }} />
    </>
  );
}