...
export default function EditCampaignModal({ campaign, onClose, onSave }: EditCampaignModalProps) {
    ...
    const fetchCampaign = async () => {
      try {
        ...
        if (data.email?.length > 0) {
          ...
          if (editorRef.current) editorRef.current.innerHTML = data.email[0].email_body || "";
        }
  ...
  const step3Valid = campaign.type === "email" ? scheduleRange[0] && scheduleRange[1] && scheduleTime : scheduleDate && scheduleTime;
  ...
  const handleUpdate = async () => {
    ...
    try {
      const start = campaign.type === "email" ? scheduleRange[0]?.format("YYYY-MM-DD") : scheduleDate;
                ...
                <div className={`form-group half ${submitted && !objective ? "error" : ""}`}>
                  ...
                  <FormControl fullWidth>
                    <Select value={objective} onChange={(e) => setObjective(e.target.value)} displayEmpty>
                      ...
                      {Object.entries(CAMPAIGN_OBJECTIVES).map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}
                    </Select>
                  </FormControl>
                </div>
                <div className={`form-group half ${submitted && !audience ? "error" : ""}`}>
                  ...
                  <FormControl fullWidth>
                    <Select value={audience} onChange={(e) => setAudience(e.target.value)} displayEmpty>
                      ...
                      {Object.entries(CAMPAIGN_AUDIENCE).map(([value, label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}
                    </Select>
                  </FormControl>
                </div>
              </div>
                ...
                <div className={`form-group half ${submitted && !startDate ? "error" : ""}`}>
                  ...
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker format="DD/MM/YYYY" value={startDate ? dayjs(startDate) : null}
                      ...
                      slots={{ openPickerIcon: CalendarTodayIcon }} slotProps={{ textField: { fullWidth: true } }} />
              ...
              <div className={`section-card ${submitted && (!subject || !emailBody) ? "error" : ""}`}>
                <div className="email-content-header">
                  ...
                  <div className="email-actions">
                    ...
                    <button className="light-btn" onClick={() => { setTemplateAttachments([]); setTemplateOpen(true); }}>+ Email Template</button>
                  </div>
                </div>
                <div className="email-body-row">
                  <div className="email-left">
                    ...
                    <div className={`form-group ${submitted && !emailBody ? "error" : ""}`}>
                      <label>Email *</label>
                      <div ref={editorRef} className="email-editor" contentEditable
                      ...
                      {templateAttachments.length > 0 && (
                        <div className="template-attachments">
                          ...
                          {templateAttachments.map((doc) => {
                            ...
                            return <div key={doc.id ? String(doc.id) : url} className="attachment-item"><a href={url} target="_blank" rel="noopener noreferrer">📎 {name}</a></div>;
                          })}
                        </div>
                      )}
                      <span className="ai-suggest">✨ AI Suggest</span>
                ...
                <div className="mode-row">
                  <div className={`mode-card ${mode === "organic" ? "selected" : ""}`} onClick={() => setMode("organic")}>
                    <div className="mode-left">
                      ...
                      <div className="mode-text"><h4>Organic Posting</h4><p>Post to your connected social accounts without ad spend.</p></div>
                    </div>
                  ...
                  <div className={`mode-card ${mode === "paid" ? "selected" : ""}`} onClick={() => setMode("paid")}>
                    <div className="mode-left">
                      ...
                      <div className="mode-text"><h4>Paid Advertising</h4><p>Boost your reach and engagement with targeted ads.</p></div>
                    </div>
                  ...
                  <div className={`schedule-field ${submitted && (!scheduleRange[0] || !scheduleRange[1]) ? "error" : ""}`}>
                    ...
                    <LocalizationProvider dateAdapter={AdapterDayjs}>
                      <div style={{ display: "flex", gap: "12px" }}>
                        <DatePicker label="From" value={scheduleRange[0]} minDate={dayjs()} maxDate={scheduleRange[1] ?? (endDate ? dayjs(endDate) : undefined)}
                          onChange={(v) => setScheduleRange([v ? dayjs(v) : null, scheduleRange[1]])} slotProps={{ textField: { fullWidth: true } }} />
                        <DatePicker label="To" value={scheduleRange[1]} minDate={scheduleRange[0] ?? dayjs()} maxDate={endDate ? dayjs(endDate) : undefined}
                          onChange={(v) => setScheduleRange([scheduleRange[0], v ? dayjs(v) : null])} slotProps={{ textField: { fullWidth: true } }} />
                      </div>
                    </LocalizationProvider>
                  </div>
