...
import { roleApi, type RoleRead, type RolePermissionPayload } from "../../../../services/role.api";
...
const UserRightsForm: React.FC<Props> = ({ onSave }) => {
  ...
  const handleSaveGrant = async () => {
    ...
    const existingApiPerms: RolePermissionPayload[] = role.apiId !== null
      ? await roleApi.list()
          .then((list) => list.find((r) => r.id === role.apiId)?.permissions ?? [])
          .catch(() => [])
      : [];
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        {mode === "edit" && activeRole && (
          <Box sx={{ border: "1px solid #E0E0E0", borderRadius: "10px", bgcolor: "#fff", p: 2, display: "flex", flexDirection: "column", gap: 1.5 }}>
            ...
            <Box sx={{ border: "1px solid #ECECEC", borderRadius: "12px", px: 2, py: 1, display: "flex", alignItems: "center" }}>
              {STEP_LABELS.map((label, idx) => (
                <React.Fragment key={label}>
                  <StepDot index={idx} label={label} active={activeStep === idx} done={idx < activeStep} />
                  {idx < STEP_LABELS.length - 1 && <Box sx={{ flex: 1, height: 1, bgcolor: "#DCDCDC", mx: 1.5 }} />}
                </React.Fragment>
              ))}
            </Box>
            ...
            <Box sx={{ display: "flex", alignItems: "center" }}>
              ...
              />
              <Box sx={{ flex: 1 }} />
              <Button variant="outlined" onClick={activeStep < 2 ? handleNext : handleSave} sx={{ textTransform: "none", minWidth: 110, borderRadius: "10px", fontSize: 14, color: "#5C5C5C", borderColor: "#C5C5C5" }}>{activeStep < 2 ? "Next" : "Save"}</Button>
            </Box>
