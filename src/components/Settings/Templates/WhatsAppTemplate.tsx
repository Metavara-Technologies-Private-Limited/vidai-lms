...
export const WhatsAppTemplateTable: React.FC = () => {
  // 1. Filter for WhatsApp type specifically
  const whatsappRows = TEMPLATES_MOCK_DATA.filter(t => t.type === 'whatsapp');

  // 2. Pagination State
  const [page, setPage] = useState(0);
  const rowsPerPage = 10;
  const totalPages = Math.ceil(whatsappRows.length / rowsPerPage);

  // 3. Slice the data for the current page
  const visibleRows = whatsappRows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      {/* Pagination Footer - Identical to Email and SMS */}
