...
export const WhatsAppTemplateTable: React.FC = () => {
  const getUseCaseStyles = (useCase: string) => {
    switch (useCase.toLowerCase()) {
      ...
      case 'marketing':
        return { color: '#22C55E', bgColor: '#DCFCE7', borderColor: '#BBF7D0' };
      case 'follow-up':
      ...
      <Table stickyHeader>
        <TableHead>
          <TableRow>
            ...
            <TableCell className={styles.headCell}>WhatsApp Message</TableCell>
        ...
        <TableBody>
          {TEMPLATES_MOCK_DATA.filter(t => t.type === 'whatsapp').map((row) => {
              ...
              <TableRow key={row.id} className={styles.bodyRow}>
                ...
                <TableCell align="right">
                  <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                    <IconButton size="small" sx={{ color: '#22C55E' }}><Visibility fontSize="inherit" /></IconButton>
