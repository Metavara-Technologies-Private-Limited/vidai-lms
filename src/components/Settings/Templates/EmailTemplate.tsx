...
export const EmailTemplateTable: React.FC<Props> = ({ useCases }) => {
  ...
  // Pagination Logic
  const visibleRows = emailRows.slice(
    page * rowsPerPage,
    page * rowsPerPage + rowsPerPage,
  );
  const pages = Array.from({ length: totalPages }, (_, i) => i + 1);

  const getUseCaseStyles = (useCaseName?: string) => {
    switch ((useCaseName || "").toLowerCase()) {
      case "appointment":
        return {
          color: "#16A34A",
          bgColor: "#F0FDF4",
      ...
      >
        <Table stickyHeader>
          ...
          <TableBody>
            {visibleRows.map((row) => {
              const useCaseName =
                useCases.find((u) => u.id === row.useCase)?.name || "";

              const ui = getUseCaseStyles(useCaseName);
              return (
