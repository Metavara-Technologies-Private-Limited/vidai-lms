...
export const EmailTemplateTable: React.FC<Props> = ({ data, searchQuery, onAction }) => {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Box className={styles.paginationWrapper} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" sx={{ ml: 'auto' }}>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p, idx) => (
            <Box
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') setPage(idx); }}
...
// helper retained here to keep file self-contained
function getUseCaseStyles(useCase: string) {
