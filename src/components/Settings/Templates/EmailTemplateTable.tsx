...
const HighlightText = ({ text, highlight }: { text: string; highlight: string }) => {
  ...
  return (
    <>
      {parts.map((part, i) => 
        regex.test(part) ? (
          <span key={i} style={{ backgroundColor: '#FEF3C7', color: '#92400E', fontWeight: 600, borderRadius: '2px' }}>
            {part}
          </span>
        ) : (part)
      )}
    ...
    <Box sx={{ width: '100%' }}>
              ...
              return (
                <TableRow key={row.id} className={styles.bodyRow}>
                  ...
                  <TableCell className={styles.subjectCell}>
                    <HighlightText text={row.subject} highlight={searchQuery} />
                  </TableCell>
                  <TableCell>
      ...
      <Box className={styles.paginationWrapper}>
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <IconButton onClick={() => setPage(page + 1)} disabled={page === totalPages - 1 || totalPages === 0} className={styles.arrowBtn}><ChevronRightIcon fontSize="small" /></IconButton>
        </Stack>
      </Box>
    </Box>
  );
