...
// import type { Campaign, SocialCampaignPayload } from "../../../types/campaigns.types";
...
//       const totalSpend = selectedPlatforms.reduce((sum, p) => sum + budgets[p.id], 0);
...
//         budget_data: {
//           ...Object.fromEntries(selectedPlatforms.map((p) => [p.id, budgets[p.id]])),
//           total: totalSpend,
//         },
//         image_url,
...
//       const res = await CampaignAPI.createSocial(payload);
...
//         type: "social",
//         status: statusValue as Campaign["status"],
//         start: startDate,
//         end: endDate,
//         platforms: accounts,
//         leads: 0,
...
//         budget_data: payload.budget_data as Record<string, number>,
//         campaign_content: firstSelectedContent,
//         image_url,
...
//                     <input ref={fileInputRefs[p.id]} type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt" hidden onChange={(e) => handleFileInsert(e, p.id)} />
//                   </React.Fragment>
//                 ))}
...
//                             <input type="number" min="0" step="10" value={budgets[p.id]}
//                               onChange={(e) => setBudget(p.id, Number(e.target.value))} className="budget-input" />
//                           </div>
//                         </div>
//                       ))}
//                     </div>
...
//                       <div>
//                         <h4>Total Budget : ${PLATFORM_LIST.filter((p) => accounts.includes(p.id)).reduce((sum, p) => sum + budgets[p.id], 0)}</h4>
