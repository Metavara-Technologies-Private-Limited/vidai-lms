...
// Keep mock data for non-doctor categories (until those APIs exist)
...
const Referrals = () => {
  ...
  // ── Fetch real doctor count from employees API ──────────────────────────
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        const leads: { assigned_to_id?: number }[] =
          Array.isArray(data) ? data : data.results ?? [];
        const uniqueDoctors = new Set(
          leads.filter((l) => l.assigned_to_id).map((l) => l.assigned_to_id)
        );
  ...
  const handleCardClick = (title: string) => {
    if (title === "Doctors") navigate("/referrals/doctors");
    if (title === "Corporate HR") navigate("/referrals/corporate");
    if (title === "Insurance Partners") navigate("/referrals/insurance");
    if (title === "Diagnostic Labs") navigate("/referrals/diagnostic");
    if (title === "Zoya") navigate("/referrals/zoya");
    if (title === "Practo") navigate("/referrals/practo");
  };
        ...
        {referralData.map((item, index) => (
          ...
          >
            ...
            <Box sx={{ display: "flex", alignItems: "baseline", gap: 0.5 }}>
              ...
              {/* Show spinner while doctor count loads, number once ready */}
              {item.title === "Doctors" && item.count === null ? (
                <CircularProgress size={12} sx={{ ml: 0.5, color: "#5392F2" }} />
              ) : (
