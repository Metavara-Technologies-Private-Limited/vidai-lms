...
export type LeadFormField = {
  ...
  stage_field_type: "text" | "number" | "date" | "dropdown";
...
export const FALLBACK_LEAD_FORM_FIELDS: LeadFormField[] = [
  ...
  ([
    ...
    model_field: String(model_field),
    field_label: String(field_label),
    field_type: field_type as LeadFormFieldType,
    stage_field_type:
      field_type === "number"
        ? "number"
        : field_type === "date"
          ? "date"
          : field_type === "dropdown" || field_type === "multi_select" || field_type === "boolean"
            ? "dropdown"
            : "text",
    options_source: String(options_source),
    form_step: Number(form_step),
...
export const LeadFormFieldAPI = {
  list: async (): Promise<LeadFormField[]> => {
    try {
      const response = await api.get<LeadFormField[]>("/lead-form-fields/");
      return response.data.length > 0 ? response.data : FALLBACK_LEAD_FORM_FIELDS;
    } catch {
      return FALLBACK_LEAD_FORM_FIELDS;
    }
  },
  create: async (
    data: Partial<LeadFormField> & { field_label: string; field_type: LeadFormFieldType },
  ): Promise<LeadFormField> => {
    const response = await api.post<LeadFormField>("/lead-form-fields/", data);
    return response.data;
  },
  update: async (
    fieldKey: string,
    data: Partial<Pick<LeadFormField, "field_label" | "field_type" | "form_step" | "section" | "sort_order" | "is_required" | "is_active">>,
  ): Promise<LeadFormField> => {
    const response = await api.patch<LeadFormField>(
      `/lead-form-fields/${encodeURIComponent(fieldKey)}/`,
      data,
    );
    return response.data;
  },
